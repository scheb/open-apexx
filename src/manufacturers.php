...
$manutypes = array('person', 'company');
...
if ( $_REQUEST['id'] && $_REQUEST['products'] ) {
	...
	//Produkte auslesen
	if ( $_REQUEST['sortby']=='release.ASC' || $_REQUEST['sortby']=='release.DESC' ) $data=$db->fetch("SELECT a.*,min(stamp) AS minrelease,IF(b.prodid IS NULL,0,1) AS isset FROM ".PRE."_products AS a LEFT JOIN ".PRE."_products_releases AS b ON a.id=b.prodid WHERE a.active='1' AND ( a.manufacturer='".$res['id']."' OR a.publisher='".$res['id']."' ) ".$where." GROUP BY a.id ".getorder($orderdef,'isset DESC',1)." ".getlimit($set['products']['manuprod_epp']));
	else $data = $db->fetch("SELECT * FROM ".PRE."_products WHERE active='1' AND ( manufacturer='".$res['id']."' OR publisher='".$res['id']."' ) ".$where.getorder($orderdef).getlimit($set['products']['manuprod_epp'])); 
	$ids = get_ids($data,'id');
...
if ( $_REQUEST['action']=='search' ) {
	...
	//SUCHE DURCHFÜHREN
		...
		foreach ( $_REQUEST['type'] AS $type ) {
			if ( in_array($type,$manutypes) ) $unittypes[]="'".$type."'";
		}
