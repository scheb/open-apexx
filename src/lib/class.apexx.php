...
class apexx
{
    ...
    {
        if ($this->is_https()) {
            $port = iif(443 != $_SERVER['SERVER_PORT'], ':'.$_SERVER['SERVER_PORT']);
    ...
    public function get_dir()
    {
        ...
        if ('/' == substr($source, -1)) {
            $source .= 'file.php';
        } //Dateiname fehlt im Pfad (=> Fehler abfangen)
        $dir = dirname($source).'/';
    ...
    public function register_functions($modulename, $info, $in = 'public')
    {
        if (!is_array($info) || !count($info)) {
            return;
        }
        if ('admin' == $in) {
            $this->functions_admin[$modulename] = $info;
        } else {
