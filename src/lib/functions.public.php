...
function pages($link, $count, $epp = 0, $varname = 'p', $pre = '')
{
    ...
    if (0 == $epp) {
        ...
        $apx->tmpl->assign(iif($pre, $pre.'_').'PAGE', [[
            'NUMBER' => 1,
            'LINK' => '#',
        ]]);
    ...
    for ($i = 1; $i <= $pages; ++$i) {
        if (false !== strpos($link, '{P}')) {
            $finallink = str_replace('{P}', $i, $link);
        } else {
            $finallink = $link.$sticky.$varname.'='.$i;
        }
        $pagedata[$i]['NUMBER'] = $i;
    ...
    if ($_REQUEST[$varname] > 1) {
        if (false !== strpos($link, '{P}')) {
            $link_previous = str_replace('{P}', $_REQUEST[$varname] - 1, $link);
        } else {
            $link_previous = $link.$sticky.$varname.'='.($_REQUEST[$varname] - 1);
        }
    }
    ...
    if ($_REQUEST[$varname] < $pages) {
        if (false !== strpos($link, '{P}')) {
            $link_next = str_replace('{P}', $_REQUEST[$varname] + 1, $link);
        } else {
            $link_next = $link.$sticky.$varname.'='.($_REQUEST[$varname] + 1);
        }
    }
    ...
    if ($_REQUEST[$varname] > 1) {
        if (false !== strpos($link, '{P}')) {
            $link_first = str_replace('{P}', '1', $link);
        } else {
            $link_first = $link.$sticky.$varname.'=1';
        }
    }
    ...
    if ($_REQUEST[$varname] < $pages) {
        if (false !== strpos($link, '{P}')) {
            $link_last = str_replace('{P}', $pages, $link);
        } else {
            $link_last = $link.$sticky.$varname.'='.$pages;
        }
    }
...
function section_filter($and = true, $fieldname = 'secid')
{
    ...
    if (is_string($user->info['section_access']) && 'all' == $user->info['section_access']) {
        $allowed = $opensec;
    } elseif (!is_array($secacc)) {
        $allowed = [];
    } else {
        $allowed = array_intersect($secacc, $opensec);
    }
...
function ordervars($orderdef, $link)
{
    ...
    foreach ($orderdef as $key => $info) {
        if ($key == $currentkey) {
            if (false !== strpos($link, '?')) {
                $sortlink = $link.'&amp;sortby='.$key.'.'.iif('ASC' == $currentord, 'DESC', 'ASC');
            } else {
                $sortlink = $link.'?sortby='.$key.'.'.iif('ASC' == $currentord, 'DESC', 'ASC');
            }
        } else {
            if (false !== strpos($link, '?')) {
                $sortlink = $link.'&amp;sortby='.$key.'.'.$info[1];
            } else {
                $sortlink = $link.'?sortby='.$key.'.'.$info[1];
            }
        }
...
function rss_replace($text, $nostrip = false)
{
    ...
    return trim(preg_replace('#( ){2,}#si', ' ', $text));
}
...
function dbcodes($text, $sig = false)
{
    ...
    if (!isset($syntax)) {
        foreach ($set['main']['codes'] as $res) {
            ...
            $syntax[] = [$find, $replace, $res['allowsig']];
        }
    }
...
function in_template($find, $parse)
{
    if (is_array($find)) {
        foreach ($find as $findme) {
            if (in_array($findme, $parse)) {
                return true;
            }
        }
    } elseif (in_array($find, $parse)) {
        return true;
    } else {
        return false;
    }
