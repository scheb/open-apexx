...
function tmessageOverlay($file, $input = [], $dir = false)
{
    global $set,$db,$apx;
    $apx->tmpl->assign_static('OVERLAY', true);
...
function choosetime($id, $empty = 0, $sel = 0)
{
    ...
    for ($i = 1; $i <= 31; ++$i) {
        $string .= '<option value="'.$i.'"'.iif($date['mday'] == $i, ' selected="selected"').'>'.sprintf('%02.d', $i).'</option>';
    }
    $string .= '</select>. ';

    //Monate
    $string .= '<select name="t_mon_'.$id.'">'.iif($empty, '<option value=""'.iif(!isset($date['mon']), ' selected="selected"').'></option>');
    $string .= '<option value="1"'.iif(1 == $date['mon'], ' selected="selected"').'>'.$apx->lang->get('MONTH_JAN').'</option>';
    $string .= '<option value="2"'.iif(2 == $date['mon'], ' selected="selected"').'>'.$apx->lang->get('MONTH_FEB').'</option>';
    $string .= '<option value="3"'.iif(3 == $date['mon'], ' selected="selected"').'>'.$apx->lang->get('MONTH_MAR').'</option>';
    $string .= '<option value="4"'.iif(4 == $date['mon'], ' selected="selected"').'>'.$apx->lang->get('MONTH_APR').'</option>';
    $string .= '<option value="5"'.iif(5 == $date['mon'], ' selected="selected"').'>'.$apx->lang->get('MONTH_MAY').'</option>';
    $string .= '<option value="6"'.iif(6 == $date['mon'], ' selected="selected"').'>'.$apx->lang->get('MONTH_JUN').'</option>';
    $string .= '<option value="7"'.iif(7 == $date['mon'], ' selected="selected"').'>'.$apx->lang->get('MONTH_JUL').'</option>';
    $string .= '<option value="8"'.iif(8 == $date['mon'], ' selected="selected"').'>'.$apx->lang->get('MONTH_AUG').'</option>';
    $string .= '<option value="9"'.iif(9 == $date['mon'], ' selected="selected"').'>'.$apx->lang->get('MONTH_SEP').'</option>';
    $string .= '<option value="10"'.iif(10 == $date['mon'], ' selected="selected"').'>'.$apx->lang->get('MONTH_OCT').'</option>';
    $string .= '<option value="11"'.iif(11 == $date['mon'], ' selected="selected"').'>'.$apx->lang->get('MONTH_NOV').'</option>';
    $string .= '<option value="12"'.iif(12 == $date['mon'], ' selected="selected"').'>'.$apx->lang->get('MONTH_DEC').'</option>';
    $string .= '</select> ';

    //Jahre
    $string .= '<select name="t_year_'.$id.'">'.iif($empty, '<option value=""'.iif(!isset($date['year']), ' selected="selected"').'></option>');
    for ($i = 2000; $i <= 2020; ++$i) {
        $string .= '<option value="'.$i.'"'.iif($date['year'] == $i, ' selected="selected"').'>'.sprintf('%02.d', $i).'</option>';
    }
    $string .= '</select> - ';
    ...
    for ($i = 0; $i <= 23; ++$i) {
        $string .= '<option value="'.$i.'"'.iif($date['hours'] === $i, ' selected="selected"').'>'.sprintf('%02.d', $i).'</option>';
    }
    $string .= '</select>:';
    ...
    for ($i = 0; $i <= 59; ++$i) {
        $string .= '<option value="'.$i.'"'.iif($date['minutes'] === $i, ' selected="selected"').'>'.sprintf('%02.d', $i).'</option>';
    }
    $string .= '</select>:';
    ...
    for ($i = 0; $i <= 59; ++$i) {
        $string .= '<option value="'.$i.'"'.iif($date['seconds'] === $i, ' selected="selected"').'>'.sprintf('%02.d', $i).'</option>';
    }
    $string .= '</select> Uhr';
...
function save_index($url, $action = false)
{
    global $apx;
    if (false === $action) {
        $action = $apx->active_module.'.'.$apx->active_action;
    }
...
function parse_tree_space($follow, $img)
{
    ...
    foreach ($follow as $id => $info) {
        ...
        $mother = $info['mother'];
        while (true) {
            if (!isset($follow[$mother]) || 1 == $follow[$mother]['level']) {
                break;
            } //Beenden, wenn kein Mutterelement existiert
            $motherinfo = $follow[$mother];
            $mother = $motherinfo['mother'];

            if ($motherinfo['next']) {
                $spacecode = $img['line'].$spacecode;
            } else {
                $spacecode = $img['space'].$spacecode;
            }
        }
...
{
    global $apx;
    if (!$apx->user->has_right('mediamanager.upload')) {
        return;
    }
...
function letters($link)
{
    ...
    if (false !== strpos($link, '{LETTER}')) {
        $fulllink = str_replace('{LETTER}', '0', $link);
    } else {
        $fulllink = $link;
    }
    ...
    foreach ($letters as $letter) {
        $selected = 0;
        if ('#' == $letter && 'spchar' == $_REQUEST['letter']) {
            $selected = 1;
        }
        if ($letter == $_REQUEST['letter']) {
            $selected = 1;
        }

        if (false !== strpos($link, '{LETTER}')) {
            $fulllink = str_replace('{LETTER}', iif('#' == $letter, 'spchar', $letter), $link);
        } else {
            $fulllink = $link.$sticky.'letter='.iif('#' == $letter, iif('#' == $letter, 'spchar', $letter), $letter);
        }
...
function section_filter($and = true, $fieldname = 'secid')
{
    ...
    if (!is_array($sections) || !count($sections)) {
        return iif($and, ' AND ').' 1 ';
    } //true
    /////// Nur gewählte Sektion
    if ($secid) {
        return iif($and, ' AND ').' ( '.$fieldname."='all' OR ".$fieldname." LIKE '%|".$secid."|%' ) ";
    }
...
function orderstr($info, $link)
{
    ...
    foreach ($info as $id => $data) {
        ...
        if ($id == $sort[0]) {
            if ('ASC' == $sort[1]) {
                $scache[] = '<a href="'.$link.'&amp;sortby='.$id.'.DESC">'.$apx->lang->get($data[2]).'</a> <img src="design/asc.gif" alt="ASC" style="vertical-align:middle;" />';
            } else {
                $scache[] = '<a href="'.$link.'&amp;sortby='.$id.'.ASC">'.$apx->lang->get($data[2]).'</a> <img src="design/desc.gif" alt="DESC" style="vertical-align:middle;" />';
            }
        } else {
            $scache[] = '<a href="'.$link.'&amp;sortby='.$id.'.'.$data[1].'">'.$apx->lang->get($data[2]).'</a>';
        }
    }
...
function array_sort_def($array, $info)
{
    ...
    return array_sort($array, $info[$info[0]][0], $info[$info[0]][1]);
}
