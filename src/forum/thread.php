        ...
        foreach ($attdata as $res) {
            if ($res['thumbnail']) {
                $attimage[$res['postid']][] = array_merge(
                    $res,
                    ['icon' => $typeicon[strtolower($mm->getext($res['name']))]]
                );
            } else {
...
if (count($data)) {
    ...
    foreach ($data as $res) {
        ...
        $postdata[$i]['FIRST'] = iif(1 == $_REQUEST['p'] && 1 == $i, 1, 0);
        ...
        //Benutzertypen
        if (!$res['userid']) {
            $postdata[$i]['IS_GUEST'] = 1;
        } elseif ('admin' == $userdat['gtype']) {
            $postdata[$i]['IS_ADMIN'] = 1;
        } elseif ('indiv' == $userdat['gtype']) {
            $postdata[$i]['IS_TEAM'] = 1;
        } elseif (in_array($res['userid'], $foruminfo['moderator'])) {
            $postdata[$i]['IS_MODERATOR'] = 1;
        }
