...
elseif ($_POST['send']) {
    ...
    if ($captchafailed) {
    ...
    } elseif (!$_POST['title'] || !$_POST['text'] || (!$user->info['userid'] && !$_POST['username']) || ('own' == $_POST['sticky_type'] && !$_POST['sticky_text'])) {
        message('back');
    } elseif ($set['forum']['spamprot'] && ($spam + $set['forum']['spamprot'] * 60) > time()) {
        message($apx->lang->get('MSG_BLOCKSPAM', ['SEC' => ($spam + $set['forum']['spamprot'] * 60) - time()]), 'back');
    } else {
        //Hash-Wert
        if (!isset($_POST['hash'])) {
        ...
        //Abonnements
        if ($user->info['userid']) {
            if (in_array($_POST['subscription'], ['none', 'instant', 'daily', 'weekly'])) {
