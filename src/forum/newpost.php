...
elseif ($_POST['send']) {
    ...
    if ($captchafailed) {
    ...
    } elseif (!$_POST['text'] || (!$user->info['userid'] && !$_POST['username'])) {
        message('back');
        ...
        if (isset($_POST['subscription']) && $user->info['userid']) {
            ...
            } elseif (in_array($_POST['subscription'], ['none', 'instant', 'daily', 'weekly'])) {
...
if ($_REQUEST['quote']) {
    ...
    if ($post['title']) {
        $_POST['title'] = iif('Re: ' == substr($post['title'], 0, 4), $post['title'], 'Re: '.$post['title']);
    }
