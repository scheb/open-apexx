...
if ('editpost' == $_REQUEST['action']) {
    ...
    //AKTION AUSFÜHREN
    elseif ($_POST['send']) {
        if (!$_POST['text'] || ($firstpost && !$_POST['title']) || (!$postinfo['userid'] && !$_POST['username']) || ($firstpost && 'own' == $_POST['sticky_type'] && !$_POST['sticky_text'])) {
            message('back');
        } else {
            //Benutzername
            if (!$postinfo['userid']) {
            ...
            if (forum_access_announce($foruminfo)) {
                if ($_POST['sticky_type'] && 'no' != $_POST['sticky_type']) {
                    $_POST['sticky'] = 1;
                    if ('announcement' == $_POST['sticky_type']) {
                        $_POST['sticky_text'] = $apx->lang->get('ANNOUNCEMENT');
                    }
                    if ('important' == $_POST['sticky_type']) {
                        $_POST['sticky_text'] = $apx->lang->get('IMPORTANT');
                    }
                } else {
                    $_POST['sticky'] = 0;
                    $_POST['sticky_text'] = '';
            ...
            if ($firstpost) {
                $_POST['icon'] = iif($_POST['icon'] && 'none' != $_POST['icon'], $_POST['icon'], -1);
        ...
        if ($firstpost) {
            //Themenicon
            if (intval($threadinfo['icon']) >= 0) {
                $_POST['icon'] = (int) $threadinfo['icon'];
            } else {
                $_POST['icon'] = 'none';
            }
    ...
    if (count($set['main']['smilies'])) {
        foreach ($set['main']['smilies'] as $res) {
            ...
            if ('/' != $res['file'][0] && defined('BASEREL')) {
                $smiledata[$si]['IMAGE'] = BASEREL.$res['file'];
            } else {
                $smiledata[$si]['IMAGE'] = $res['file'];
            }
            if (16 == $si) {
                break;
            }
    ...
    if (count($data)) {
        foreach ($data as $res) {
            $filetypes[] = $res['ext'];
            $typeinfo[$res['ext']] = [$res['size'] * 1024, $res['icon']];
        }
    }
    ...
    $apx->tmpl->assign('ICON', iif('none' === $_POST['icon'], $_POST['icon'], (int) $_POST['icon']));
...
elseif ('delpost' == $_REQUEST['action']) {
    $apx->lang->drop('admin');
...
elseif ('delposts' == $_REQUEST['action']) {
    $apx->lang->drop('admin');
    $apx->lang->drop('postform');
...
elseif ('mergeposts' == $_REQUEST['action']) {
    $apx->lang->drop('admin');
    $apx->lang->drop('postform');
...
elseif ('recoverpost' == $_REQUEST['action']) {
    $apx->lang->drop('admin');
    ...
    if (!forum_access_recoverpost($foruminfo, $threadinfo, $postinfo)) {
        tmessage('noright', [], false, false);
    }
...
elseif ('delthread' == $_REQUEST['action']) {
    $apx->lang->drop('admin');
...
elseif ('recoverthread' == $_REQUEST['action']) {
    $apx->lang->drop('admin');
...
elseif ('closethread' == $_REQUEST['action']) {
    $apx->lang->drop('admin');
...
elseif ('openthread' == $_REQUEST['action']) {
    $apx->lang->drop('admin');
...
elseif ('movethread' == $_REQUEST['action']) {
    $apx->lang->drop('admin');
    ...
    if ($_POST['moveto'] && $_POST['moveto'] != $foruminfo['forumid']) {
        ...
        if (!forum_access_visible($destinfo)) {
            tmessage('noright', [], false, false);
        }
        if (!forum_access_read($destinfo)) {
            tmessage('noright', [], false, false);
        }
    }
    ...
    if ($destinfo['forumid']) {
        ...
        tmessage('movethread', ['ID' => $threadinfo['threadid'], 'FORUM' => $forumdata]);
    }
}

// Thema teilen

elseif ('splitthread' == $_REQUEST['action']) {
    $apx->lang->drop('admin');
    $apx->lang->drop('postform');
    ...
    //ABSENDEN
    if ($_POST['send']) {
        $do = 'merge' == $_POST['do'] ? 'merge' : 'new';

        //Prüfen, ob Zugang zum Zielforum
        if ('new' == $do && $_POST['forum']) {
        ...
        if ('merge' == $do && $_POST['targetid']) {
            $destinfo = thread_info($_POST['targetid']);
            if (!$destinfo['threadid']) {
                message($apx->lang->get('MSG_THREADNOTEXIST'));
            }
            $destforuminfo = forum_info($destinfo['forumid']);
            if (!forum_access_visible($destforuminfo)) {
                tmessage('noright', [], false, false);
            }
            if (!forum_access_read($destforuminfo)) {
                tmessage('noright', [], false, false);
            }
        }
        ...
        if ('new' == $do && $total <= $selected) {
            message($apx->lang->get('MSG_NOTALLPOSTS'), 'back');
        } elseif ('new' == $do && (!$_POST['forum'] || !$_POST['title'] || !$_POST['post'] || !count($_POST['post']))) {
            message($apx->lang->get('CORE_BACK'), 'back');
        } elseif ('merge' == $do && !$_POST['targetid']) {
            message($apx->lang->get('CORE_BACK'), 'back');
        } else {
            ...
            //NEUES THEMA ERSTELLEN
                ...
                //Sticky
                if (forum_access_announce($foruminfo) && $_POST['sticky_type'] && 'no' != $_POST['sticky_type']) {
                    $_POST['sticky'] = 1;
                    if ('announcement' == $_POST['sticky_type']) {
                        $_POST['sticky_text'] = $apx->lang->get('ANNOUNCEMENT');
                    }
...
elseif ('ipstats' == $_REQUEST['action']) {
