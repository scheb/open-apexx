...
if ($_REQUEST['send']) {
    ...
    if ($_POST['threadid']) {
        ...
        $_POST['forumid'] = [$threadinfo['forumid']];
    }

    //Standardeinstellungen
    if (!isset($_POST['keywords_req'])) {
        $_POST['keywords_req'] = '';
    }
    if (!isset($_POST['keywords_one'])) {
        $_POST['keywords_one'] = '';
    }
    if (!isset($_POST['keywords_not'])) {
        $_POST['keywords_not'] = '';
    }
    ...
    if (!isset($_POST['searchsubforums'])) {
        $_POST['searchsubforums'] = 1;
    }
    if (!isset($_POST['period'])) {
        $_POST['period'] = 'all';
    }
    if (!isset($_POST['period_dir'])) {
        $_POST['period_dir'] = 'younger';
    }
    if (!isset($_POST['sortby'])) {
        $_POST['sortby'] = 'lastpost';
    }
    if (!isset($_POST['sortby_dir'])) {
        $_POST['sortby_dir'] = 'DESC';
    }
    if (!isset($_POST['display'])) {
        $_POST['display'] = 'threads';
    }
    ...
    if (
        ...
        foreach ($data as $res) {
            ...
            if ('all' != $_POST['forumid'][0] && !in_array($res['forumid'], $_POST['forumid']) && $res['level'] <= $searchsubtill) {
                continue;
            }
            //Unterforen ebenfalls durchsuchen
            if ($_POST['searchsubforums'] && $searchsubtill > $res['level']) {
                $searchsubtill = $res['level'];
            }
        ...
        if (count($keywords)) {
            ...
            if ($_POST['searchtitle'] && !$_POST['searchtext']) {
                $searchindex = ' ( istitle=1 AND ( '.$searchindex.' ) ) ';
            } elseif (!$_POST['searchtitle'] && $_POST['searchtext']) {
                $searchindex = ' ( istitle=0 AND ( '.$searchindex.' ) ) ';
            } else {
        ...
        if ('posts' == $_POST['display']) {
            ...
            if (isset($keyword_threads)) {
                $inthread = array_intersect($inthread, $keyword_threads);
            }
            if (!count($inthread)) {
                $inthread = [-1];
            }
            ...
            if ($_REQUEST['author'] && $_POST['findposts']) {
                if ($_POST['exact']) {
                    $wherepost .= iif($wherepost, ' AND ')." username='".addslashes($_REQUEST['author'])."' ";
                } else {
                    $wherepost .= iif($wherepost, ' AND ')." username LIKE '%".addslashes($_REQUEST['author'])."%' ";
                }
            }

            //Zeitraum
            if ($_POST['period']) {
                $op = iif('older' == $_POST['period_dir'], '<=', '>=');
                if ('lastvisit' == $_POST['period']) {
                    $wherepost .= iif($wherepost, ' AND ').' time'.$op."'".$user->info['forum_lastvisit']."'";
                } elseif ('yesterday' == $_POST['period']) {
                    $wherepost .= iif($wherepost, ' AND ').' time'.$op."'".(time() - 24 * 3600)."'";
                } elseif ('week1' == $_POST['period']) {
                    $wherepost .= iif($wherepost, ' AND ').' time'.$op."'".(time() - 7 * 24 * 3600)."'";
                } elseif ('week2' == $_POST['period']) {
                    $wherepost .= iif($wherepost, ' AND ').' time'.$op."'".(time() - 14 * 24 * 3600)."'";
                } elseif ('month1' == $_POST['period']) {
                    $wherepost .= iif($wherepost, ' AND ').' time'.$op."'".(time() - 30 * 24 * 3600)."'";
                } elseif ('month3' == $_POST['period']) {
                    $wherepost .= iif($wherepost, ' AND ').' time'.$op."'".(time() - 90 * 24 * 3600)."'";
                } elseif ('month6' == $_POST['period']) {
                    $wherepost .= iif($wherepost, ' AND ').' time'.$op."'".(time() - 180 * 24 * 3600)."'";
                } elseif ('year' == $_POST['period']) {
                    $wherepost .= iif($wherepost, ' AND ').' time'.$op."'".(time() - 365 * 24 * 3600)."'";
                }
            }
        ...
        /////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if ($wherepost) {
                ...
                if (!is_array($inthread) || !count($inthread)) {
                    $inthread = [-1];
                }
            ...
            if ($_REQUEST['author'] && $_POST['findthreads'] && !$_POST['findposts']) {
                if ($_POST['exact']) {
                    ...
                    $wherethread .= iif($wherethread, ' AND ')." opener LIKE '%".addslashes($_REQUEST['author'])."%' ";
                }
            }
