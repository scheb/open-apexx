...
if (!$_REQUEST['getid'] && !$_POST['postid'] && !$_REQUEST['hash']) {
    die('invalid use of attachments!');
}
...
if ($_REQUEST['getid']) {
    ...
    if (!forum_access_readattachment($foruminfo)) {
        tmessage('noright', [], false, false);
    }
    ...
    if (!$att['mime']) {
        if ('GIF' == $ext) {
            $att['mime'] = 'image/gif';
        } elseif ('JPG' == $ext || 'JPE' == $ext || 'JPEG' == $ext) {
            $att['mime'] = 'image/jpeg';
        } elseif ('PNG' == $ext) {
            $att['mime'] = 'image/png';
        } else {
    ...
    //Datei ausgeben
    if (headers_sent()) {
        die('Some data has already been output to browser, can not send file!');
    }
...
if ($_POST['send']) {
    for ($i = 1; $i <= 3; ++$i) {
        ...
        if (!isset($typeinfo[$ext])) { //Nicht erlaubt
            if ($message) {
                $message .= '<br />';
            }
            $message .= $apx->lang->get('MSG_NOUPLOAD', ['FILE' => $file['name']]).' ';
            $message .= $apx->lang->get('MSG_WRONGTYPE');

            continue;
        }
        if ($file['size'] > $typeinfo[$ext][0]) {
            if ($message) {
                $message .= '<br />';
            }
            $message .= $apx->lang->get('MSG_NOUPLOAD', ['FILE' => $file['name']]).' ';
            $message .= $apx->lang->get('MSG_TOOBIG', ['MAXSIZE' => $typeinfo[$ext][0]]);
        ...
        $message .= $apx->lang->get('MSG_OK', ['FILE' => $file['name']]);
    }
}
...
if ($_POST['delete'] && is_array($_POST['delete'])) {
    ...
    if ($delete) {
        ...
        $message .= $apx->lang->get('MSG_DELETE', ['FILE' => $filename]);
    }
}
