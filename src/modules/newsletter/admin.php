...
class action
{
	...
	');
        ...
        $orderdef[0] = 'addtime';
        $orderdef['subject'] = ['subject', 'ASC', 'COL_SUBJECT'];
        $orderdef['addtime'] = ['addtime', 'DESC', 'SORT_ADDTIME'];
        $orderdef['sendtime'] = ['sendtime', 'DESC', 'COL_SENDTIME'];

        $col[] = ['&nbsp;', 1, ''];
        $col[] = ['COL_SUBJECT', 50, 'class="title"'];
        $col[] = ['COL_CATEGORY', 30, 'align="center"'];
        $col[] = ['COL_SENDTIME', 20, 'align="center"'];
    ...
    public function add()
    {
        ...
        if (1 == $_POST['send']) {
            if (!checkToken()) {
                infoInvalidToken();
            } elseif (!$_POST['subject'] || !$_POST['text'] || !isset($_POST['catid']) || !$_POST['text_html']) {
                infoNotComplete();
            } else {
                $_POST['addtime'] = time();
    ...
    public function addnews()
    {
        ...
        if (!$_POST['period'] && !$_POST['finish'] && !$_POST['send']) {
            ...
            if ('day' == $_POST['type'] && $_POST['day_count']) {
                $_POST['period'] = $_POST['day_count'] * 24 * 3600;
            } elseif ('week' == $_POST['type'] && $_POST['week_count']) {
                $_POST['period'] = $_POST['week_count'] * 7 * 24 * 3600;
            } elseif ('month' == $_POST['type'] && $_POST['month_count']) {
    ...
    public function edit()
    {
        ...
        if (1 == $_POST['send']) {
            if (!checkToken()) {
                infoInvalidToken();
            } elseif (!$_POST['subject'] || !isset($_POST['catid']) || !$_POST['text'] || !$_POST['text_html']) {
                infoNotComplete();
            } else {
    ...
    public function send()
    {
        ...
        if (!$_REQUEST['doit']) {
            list($title) = $db->first('SELECT subject FROM '.PRE."_newsletter WHERE id='".$_REQUEST['id']."' LIMIT 1");
            $apx->tmpl->assign('MESSAGE', $apx->lang->get('MSG_SEND', ['TITLE' => compatible_hsc($title)]));
            tmessageOverlay('send', ['ID' => $_REQUEST['id']]);
        ...
        if (!isset($_REQUEST['done'])) {
            tmessageOverlay('sending', ['FORWARDER' => 'action.php?action=newsletter.send&amp;id='.$_REQUEST['id'].'&amp;doit=1&amp;done=0']);
	...
	');
        if (count($data)) {
            ...
            if ($newsletter['addsig']) {
                $minput['{SIGNATURE}'] = $set['newsletter']['sig_html'];
            } else {
                $minput['{SIGNATURE}'] = '';
            }
            $html_template = stripslashes(implode('', file(BASEDIR.getpath('tmpl_modules_public', ['MODULE' => 'newsletter', 'THEME' => 'default']).'html_newsletter.html')));
            ...
            tmessageOverlay('sending', ['FORWARDER' => 'action.php?action=newsletter.send&amp;id='.$_REQUEST['id'].'&amp;doit=1&amp;done='.($done + $countPerCall)]);
    ...
    public function sendNewsletter($res, $newsletter)
    {
        global $set,$db,$apx;
        if ($res['html']) {
            mail(
    ...
    public function preview()
    {
        ...
        if (isset($_POST['sendto']) && checkmail($_POST['sendto'])) {
            ...
            tmessageOverlay('preview', ['ID' => $_REQUEST['id']]);
        }
    }
    ...
    public function eshow()
    {
        ...
        $layerdef[] = ['LAYER_ALL', 'action.php?action=newsletter.eshow', !$_REQUEST['what']];
        $layerdef[] = ['LAYER_INACTIVE', 'action.php?action=newsletter.eshow&amp;what=inactive', 'inactive' == $_REQUEST['what']];
    ...
    //Einträge auflisten
    public function eshow_print($data)
    {
        global $set,$db,$apx,$html;

        $col[] = ['', 1, ''];
        $col[] = ['COL_EMAIL', 70, 'class="title"'];
        $col[] = ['COL_CATEGORIES', 30, 'style="text-align:center;"'];
        ...
        if (count($data)) {
            foreach ($data as $res) {
                ...
                if ($totalCount == $inactiveCount) {
                    $tabledata[$i]['COL1'] = '<img src="design/reddot.gif" alt="'.$apx->lang->get('CORE_INACTIVE').'" title="'.$apx->lang->get('CORE_INACTIVE').'" />';
                } elseif ($inactiveCount) {
                    $tabledata[$i]['COL1'] = '<img src="design/greendotwait.gif" alt="'.$apx->lang->get('CORE_INACTIVE').'" title="'.$apx->lang->get('CORE_INACTIVE').'" />';
                } else {
                ...
                //Optionen
                if ($apx->user->has_right('newsletter.eedit')) {
                    $tabledata[$i]['OPTIONS'] .= optionHTML('edit.gif', 'newsletter.eedit', 'id='.$res['id'], $apx->lang->get('CORE_EDIT'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($apx->user->has_right('newsletter.edel')) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('del.gif', 'newsletter.edel', 'id='.$res['id'], $apx->lang->get('CORE_DEL'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($inactiveCount && $apx->user->has_right('newsletter.eenable')) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('enable.gif', 'newsletter.eenable', 'id='.$res['id'], $apx->lang->get('CORE_ENABLE'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }
            }
        ...
        $multiactions = [];
        if ($apx->user->has_right('newsletter.edel')) {
            $multiactions[] = [$apx->lang->get('CORE_DEL'), 'action.php?action=newsletter.edel', false];
        }
        if ($apx->user->has_right('newsletter.eenable')) {
            $multiactions[] = [$apx->lang->get('CORE_ENABLE'), 'action.php?action=newsletter.eenable', false];
        }
    ...
    public function eadd()
    {
        ...
        if ('all' == $_POST['catid'][0] || !isset($_POST['catid'])) {
            $_POST['catid'] = ['all'];
        }
        ...
        if (1 == $_POST['send']) {
            ...
            if (!checkToken()) {
                infoInvalidToken();
            } elseif (!$_POST['email'] || !$_POST['catid']) {
                infoNotComplete();
            } elseif (!checkmail($_POST['email'])) {
                info($apx->lang->get('INFO_WRONGSYNTAX'));
            } elseif ($check) {
            ...
            $catinfo = $set['newsletter']['categories'];
            asort($catinfo);
            $catlist = '<option value="all"'.iif('all' == $_POST['catid'][0], 'selected="selected"').' style="font-weight:bold;">'.$apx->lang->get('ALL').'</option>';
            foreach ($catinfo as $id => $name) {
    ...
    public function eenable()
    {
        ...
        if (is_array($_REQUEST['multiid'])) {
            if (!checkToken()) {
                ...
                if (count($cache)) {
                    ...
                    foreach ($cache as $id) {
                        logit('NEWSLETTER_EENABLE', 'ID #'.$id);
                    }
                }
        ...
        //Einzeln
            ...
            if (1 == $_POST['send']) {
                ...
                list($title) = $db->first('SELECT email FROM '.PRE."_newsletter_emails WHERE id='".$_REQUEST['id']."' LIMIT 1");
                $apx->tmpl->assign('MESSAGE', $apx->lang->get('MSG_TEXT', ['TITLE' => compatible_hsc($title)]));
                tmessageOverlay('eenable', ['ID' => $_REQUEST['id']]);
            }
        }
    }
    ...
    public function eimport()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            if (!checkToken()) {
                ...
                info($apx->lang->get('INFO_WRONGSYNTAX', ['EMAILS' => implode(', ', $wrongmails)]));
            } else {
                //Kategorien
                if ('all' == $_POST['catid'][0]) {
                    $catids = array_keys($set['newsletter']['categories']);
    ...
    public function catshow()
    {
        ...
        if ('edit' == $_REQUEST['do'] && isset($data[$_REQUEST['id']])) {
            if ($_POST['send']) {
                if (!checkToken()) {
                    infoInvalidToken();
                } elseif (!$_POST['title']) {
