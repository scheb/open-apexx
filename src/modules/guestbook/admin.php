...
class action
{
    ...
    public function show()
    {
        ...
        $col[] = ['COL_NAME', 30, 'class="title"'];
        $col[] = ['COL_TEXT', 50, ''];
        $col[] = ['COL_IP', 20, 'align="center"'];
        ...
        $multiactions = [];
        if ($apx->user->has_right('guestbook.del')) {
            $multiactions[] = [$apx->lang->get('CORE_DEL'), 'action.php?action=guestbook.del&module='.$this->module.'&mid='.$this->mid];
        }
        if ($set['guestbook']['mod']) {
            if ($apx->user->has_right('guestbook.enable')) {
                $multiactions[] = [$apx->lang->get('CORE_ENABLE'), 'action.php?action=guestbook.enable&module='.$this->module.'&mid='.$this->mid];
            }
            if ($apx->user->has_right('guestbook.disable')) {
                $multiactions[] = [$apx->lang->get('CORE_DISABLE'), 'action.php?action=guestbook.disable&module='.$this->module.'&mid='.$this->mid];
            }
        }
    ...
    public function blockip()
    {
        ...
        if ('add' == $_REQUEST['do']) {
            if (!checkToken()) {
                ...
                $set['guestbook']['blockip'][] = ['startip' => $start, 'endip' => $end];
                $db->query('UPDATE '.PRE."_config SET value='".addslashes(serialize($set['guestbook']['blockip']))."' WHERE module='guestbook' AND varname='blockip' LIMIT 1");
                printJSRedirect('action.php?action=guestbook.blockip');
            }
    ...
    public function blockcontent()
    {
        ...
        if ('del' == $_REQUEST['do']) {
            if ($_POST['send']) {
                ...
                $apx->tmpl->assign('MESSAGE', $apx->lang->get('MSG_DEL', ['TITLE' => compatible_hsc($set['guestbook']['blockstring'][$_REQUEST['id']])]));
        ...
        $col[] = ['TITLE_GUESTBOOK_BLOCKCONTENT', 100, 'class="title"'];
