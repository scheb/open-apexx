...
class action
{
    ...
    public function login()
    {
        ...
        if ($_POST['send']) {
            if ($_POST['login_user'] && $_POST['login_pwd']) {
                ...
                } elseif ('admin' != $res['gtype'] && 'indiv' != $res['gtype']) {
    ...
    public function logout()
    {
        ...
        if (!checkToken()) {
            ...
            if (!$apx->user->info['userid']) {
                message($apx->lang->get('MSG_NOLOGIN'), 'action.php');
            } else {
    ...
    public function show()
    {
        ...
        if (($_REQUEST['item'] && ($_REQUEST['name'] || $_REQUEST['profile'])) || $_POST['sgroupid']) {
            ...
            $ids[] = -1;
            $searchid = saveSearchResult('admin_user', $ids, [
                'item' => $_REQUEST['item'],
                'name' => $_REQUEST['name'],
        ...
        $layerdef[] = ['LAYER_TEAM', 'action.php?action=user.show', !$_REQUEST['who']];
        $layerdef[] = ['LAYER_ALL', 'action.php?action=user.show&amp;who=all', 'all' == $_REQUEST['who']];
        $layerdef[] = ['LAYER_ACTIVATE', 'action.php?action=user.show&amp;who=activate', 'activate' == $_REQUEST['who']];

        //Layer Header ausgeben
        $html->layer_header($layerdef);

        $orderdef[0] = 'user_login';
        $orderdef['active'] = ['a.active', 'DESC', 'COL_ACTIVE'];
        $orderdef['user_login'] = ['a.username_login', 'ASC', 'COL_USER_LOGIN'];
        $orderdef['user'] = ['a.username', 'ASC', 'COL_USER'];
        $orderdef['regtime'] = ['a.reg_time', 'DESC', 'COL_REGTIME'];
        $orderdef['lastactive'] = ['a.lastactive', 'DESC', 'COL_LASTACTIVE'];
        $orderdef['group'] = ['b.name', 'ASC', 'COL_GROUP'];
    ...
    public function show_print($data)
    {
        ...
        $col[] = ['', 1, 'align="center"'];
        $col[] = ['COL_USER_LOGIN', 20, 'class="title"'];
        $col[] = ['COL_USER', 20, 'align="center"'];
        $col[] = ['COL_GROUP', 30, 'align="center"'];
        $col[] = ['COL_LASTACTIVE', 20, 'align="center"'];
        ...
        if (count($data)) {
            foreach ($data as $res) {
                ...
                if ($res['userid'] != $apx->user->info['userid']) {
                    $tabledata[$obj]['ID'] = $res['userid'];
                }

                if ($res['active']) {
                    $tabledata[$obj]['COL1'] = '<img src="design/active.gif" title="'.$apx->lang->get('CORE_ACTIVE').'" alt="'.$apx->lang->get('CORE_ACTIVE').'" />';
                } else {
                    $tabledata[$obj]['COL1'] = '<img src="design/inactive.gif" title="'.$apx->lang->get('CORE_INACTIVE').'" alt="'.$apx->lang->get('CORE_INACTIVE').'" />';
                }
                ...
                if ($res['userid'] != $apx->user->info['userid'] && $apx->user->has_right('user.edit')) {
                    $tabledata[$obj]['OPTIONS'] .= optionHTML('edit.gif', 'user.edit', 'id='.$res['userid'], $apx->lang->get('CORE_EDIT'));
                } else {
                    $tabledata[$obj]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($res['userid'] != $apx->user->info['userid'] && $apx->user->has_right('user.del')) {
                    $tabledata[$obj]['OPTIONS'] .= optionHTMLOverlay('del.gif', 'user.del', 'id='.$res['userid'], $apx->lang->get('CORE_DEL'));
                } else {
                    $tabledata[$obj]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($res['userid'] != $apx->user->info['userid'] && $apx->user->has_right('user.enable') && 'BYADMIN' == $res['reg_key']) {
                    $tabledata[$obj]['OPTIONS'] .= optionHTML('enable.gif', 'user.enable', 'id='.$res['userid'].'&sectoken='.$apx->session->get('sectoken'), $apx->lang->get('CORE_ENABLE'));
                } else {
                    $tabledata[$obj]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($apx->user->has_right('user.profile')) {
                    $tabledata[$obj]['OPTIONS'] .= optionHTML('details.gif', 'user.profile', 'id='.$res['userid'], $apx->lang->get('PROFILE'));
                } else {
                    $tabledata[$obj]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($set['user']['blog'] && $blogcount[$res['userid']]['count'] && $apx->user->has_right('user.blog')) {
                    $tabledata[$obj]['OPTIONS'] .= optionHTML('blog.gif', 'user.blog', 'userid='.$res['userid'], $apx->lang->get('BLOG'));
                } else {
                    $tabledata[$obj]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($set['user']['guestbook'] && $gbcount[$res['userid']]['count'] && $apx->user->has_right('user.guestbook')) {
                    $tabledata[$obj]['OPTIONS'] .= optionHTML('comments.gif', 'user.guestbook', 'userid='.$res['userid'], $apx->lang->get('GUESTBOOK'));
                } else {
                    $tabledata[$obj]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($set['user']['gallery'] && $galcount[$res['userid']]['count'] && $apx->user->has_right('user.gallery')) {
                    $tabledata[$obj]['OPTIONS'] .= optionHTML('pic.gif', 'user.gallery', 'userid='.$res['userid'], $apx->lang->get('GALLERY'));
                } else {
                    $tabledata[$obj]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if (!$tabledata[$obj]['OPTIONS']) {
                    $tabledata[$obj]['OPTIONS'] .= '&nbsp;';
                }
            }
        }
        ...
        $multiactions = [];
        if ($apx->user->has_right('user.edit')) {
            $multiactions[] = [$apx->lang->get('MULTI_EDIT'), 'action.php?action=user.edit', true];
        }
    ...
    public function add()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            if (!checkToken()) {
                infoInvalidToken();
            } elseif (!$_POST['username_login'] || !$_POST['username'] || !$_POST['pwd1'] || !$_POST['pwd2'] || !$_POST['email']) {
                infoNotComplete();
            } elseif ($_POST['pwd1'] != $_POST['pwd2']) {
                info($apx->lang->get('INFO_PWNOMATCH'));
            } elseif ($usercheck) {
                info($apx->lang->get('INFO_USEREXISTS'));
        ...
        //Erster Durchlauf
            ...
            if (count($data)) {
                foreach ($data as $res) {
                    $grouplist .= '<option value="'.$res['groupid'].'"'.iif($_POST['groupid'] == $res['groupid'] || !isset($_POST['groupid']) && 2 == $res['groupid'], ' selected="selected"').'>'.replace($res['name']).'</option>';
                }
            }
    ...
    public function edit()
    {
        ...
        if (is_array($_REQUEST['multiid'])) {
            ...
            $ids = array_diff($ids, [$apx->user->info['userid']]);
            if (!count($ids)) {
            ...
            if (1 == $_POST['send'] && intval($_POST['groupid'])) {
                if (!checkToken()) {
                    printInvalidToken();
                } else {
                    if (count($ids)) {
        ...
        //Einzeln
            ...
            if (1 == $_POST['send']) {
                ...
                if (!checkToken()) {
                    infoInvalidToken();
                } elseif (!$_POST['id'] || !$_POST['username_login'] || !$_POST['username'] || (($_POST['pwd1'] || $_POST['pwd2']) && (!$_POST['pwd1'] || !$_POST['pwd2'])) || !$_POST['email']) {
                    infoNotComplete();
                } elseif ($_POST['pwd1'] != $_POST['pwd2']) {
                    info($apx->lang->get('INFO_PWNOMATCH'));
                } elseif ($usercheck) {
            ...
            //Erster Durchlauf
                ...
                $ex = ['userid', 'password', 'birthday', 'reg_time', 'reg_email', 'lastonline', 'lastactive'];
    ...
    public function del()
    {
        ...
        if ($_REQUEST['id'] == $apx->user->info['userid']) {
            die('can not delete yourself!');
        }
    ...
    public function enable()
    {
        ...
        if (!checkToken()) {
            ...
            if ('BYADMIN' != $res['reg_key']) {
                die('can not activate user!');
            }
    ...
    public function blog()
    {
        ...
        $orderdef[0] = 'pub';
        $orderdef['title'] = ['title', 'ASC', 'TITLE'];
        $orderdef['pub'] = ['time', 'DESC', 'COL_PUB'];

        if ($_REQUEST['userid']) {
            $col[] = ['TITLE', 85, 'class="title"'];
            $col[] = ['COL_PUB', 15, 'align="center"'];
        } else {
            $col[] = ['TITLE', 65, 'class="title"'];
            $col[] = ['COL_PUB', 15, 'align="center"'];
            $col[] = ['COL_OWNER', 20, 'align="center"'];
        }
        ...
        //ÜBERSICHT
        if ($_REQUEST['userid']) {
            $ownerfilter = " AND userid='".$_REQUEST['userid']."' ";
        } else {
            $ownerfilter = '';
        }
            ...
            foreach ($data as $res) {
                ...
                if ($apx->is_module('comments')) {
                    ...
                    if ($comments && $apx->is_module('comments') && $res['allowcoms'] && $apx->user->has_right('comments.show')) {
                        $tabledata[$i]['OPTIONS'] .= optionHTML('comments.gif', 'comments.show', 'module=userblog&mid='.$res['id'], $apx->lang->get('COMMENTS').' ('.$comments.')');
                    } else {
    ...
    public function blog_del()
    {
        ...
        if ($_POST['send']) {
            ...
            $apx->tmpl->assign('MESSAGE', $apx->lang->get('MSG_TEXT', ['TITLE' => compatible_hsc($title)]));
            tmessageOverlay('blogdel', ['ID' => $_REQUEST['id'], 'USERID' => $_REQUEST['userid']]);
        }
    }
    ...
    public function guestbook()
    {
        ...
        //AKTIONEN
        if ('edit' == $_REQUEST['do']) {
            return $this->guestbook_edit();
        }
        if ('del' == $_REQUEST['do']) {
            return $this->guestbook_del();
        }
        $orderdef[0] = 'time';
        $orderdef['name'] = ['username', 'ASC', 'COL_NAME'];
        $orderdef['time'] = ['time', 'DESC', 'COL_ADDTIME'];

        if ($_REQUEST['userid']) {
            $col[] = ['COL_NAME', 20, 'class="title"'];
            $col[] = ['COL_TEXT', 65, ''];
            $col[] = ['COL_ADDTIME', 15, 'align="center"'];
        } else {
            $col[] = ['COL_NAME', 20, 'class="title"'];
            $col[] = ['COL_TEXT', 45, ''];
            $col[] = ['COL_ADDTIME', 15, 'align="center"'];
    ...
    public function guestbook_del()
    {
        ...
        if ($_POST['send']) {
            ...
            $apx->tmpl->assign('MESSAGE', $apx->lang->get('MSG_TEXT', ['TITLE' => compatible_hsc($title)]));
            tmessageOverlay('guestbookdel', ['ID' => $_REQUEST['id'], 'USERID' => $_REQUEST['userid']]);
        }
    }
    ...
    public function gallery()
    {
        ...
        $orderdef[0] = 'update';
        $orderdef['name'] = ['title', 'ASC', 'COL_TITLE'];
        $orderdef['addtime'] = ['addtime', 'DESC', 'COL_ADDTIME'];
        $orderdef['update'] = ['lastupdate', 'DESC', 'COL_LASTUPDATE'];

        if ($_REQUEST['userid']) {
            $col[] = ['COL_TITLE', 60, ''];
            $col[] = ['COL_PICS', 10, 'align="center"'];
            $col[] = ['COL_ADDTIME', 15, 'align="center"'];
            $col[] = ['COL_LASTUPDATE', 15, 'align="center"'];
        } else {
            $col[] = ['COL_TITLE', 40, 'class="title"'];
            $col[] = ['COL_PICS', 10, 'align="center"'];
            $col[] = ['COL_ADDTIME', 15, 'align="center"'];
            $col[] = ['COL_LASTUPDATE', 15, 'align="center"'];
            $col[] = ['COL_OWNER', 20, 'align="center"'];
    ...
    public function gallery_del()
    {
        ...
        if ($_POST['send']) {
            ...
            $apx->tmpl->assign('MESSAGE', $apx->lang->get('MSG_TEXT', ['TITLE' => compatible_hsc($title)]));
            tmessageOverlay('gallerydel', ['ID' => $_REQUEST['id'], 'USERID' => $_REQUEST['userid']]);
        }
    }
    ...
    public function gallery_pics()
    {
        ...
        $orderdef[0] = 'time';
        $orderdef['time'] = ['id', 'DESC', 'COL_ADDTIME'];
        $orderdef['caption'] = ['caption', 'ASC', 'COL_CAPTION'];

        $col[] = ['COL_THUMBNAIL', 20, 'align="center"'];
        $col[] = ['COL_CAPTION', 80, ''];
    ...
    //PICS EDIT
    public function gallery_pics_edit()
    {
        global $set,$apx,$db,$html;
        $_REQUEST['userid'] = (int) $_REQUEST['userid'];
        $_REQUEST['galid'] = (int) $_REQUEST['galid'];
    ...
    //PICS DEL
    public function gallery_pics_del()
    {
        global $set,$apx,$db,$html;
        $_REQUEST['userid'] = (int) $_REQUEST['userid'];
        $_REQUEST['galid'] = (int) $_REQUEST['galid'];
    ...
    public function gshow()
    {
        ...
        $orderdef[0] = 'group';
        $orderdef['group'] = ['name', 'ASC', 'COL_GROUP'];

        $col[] = ['COL_GROUP', 75, 'class="title"'];
        $col[] = ['COL_USERS', 25, 'align="center"'];
    ...
    public function gadd()
    {
        ...
        if (1 == $_POST['send']) {
            if (!in_array($_POST['gtype'], ['admin', 'indiv', 'public', 'guest'])) {
                $_POST['gtype'] = 'public';
            }
            ...
            if (!checkToken()) {
                infoInvalidToken();
            } elseif (!$_POST['name'] || ('indiv' == $_POST['gtype'] && !$_POST['right'])) {
                infoNotComplete();
            } elseif (count($apx->sections) && (!count($_POST['section_access']) || ('indiv' == $_POST['gtype'] && !count($_POST['section_access'])))) {
                infoNotComplete();
            } elseif ($checkname) {
                info($apx->lang->get('INFO_GROUPEXISTS'));
            } else {
                //INDIV
                if ('indiv' == $_POST['gtype']) {
                    $newr = [];
                    $newsp = [];
                    ...
                    //Sektionen
                    if ('all' == $_POST['section_access'][0]) {
                        $section_access = 'all';
                    } else {
                        $section_access = serialize($_POST['section_access']);
                    }
                }
            ...
            if (is_array($apx->sections) && count($apx->sections)) {
                if (!isset($_POST['section_access']) || 'all' == $_POST['section_access'][0]) {
                    $_POST['section_access'] = ['all'];
                }
                $section_access = '<option value="all"'.iif('all' == $_POST['section_access'][0], ' selected="selected"').' style="font-weight:bold;">'.$apx->lang->get('ALLSEC').'</option>';
    ...
    public function gedit()
    {
        ...
        //Erster Durchlauf!
            ...
            if ('all' == $res['section_access']) {
                $_POST['section_access'][0] = 'all';
            } else {
                $_POST['section_access'] = unserialize($res['section_access']);
            }
            ...
            if ('indiv' == $res['gtype']) {
                ...
                foreach ($rights as $right) {
                    $_POST['right'][$right] = true;
                }
                if (is_array($sprights)) {
                    foreach ($sprights as $spright) {
                        $_POST['spright'][$spright] = true;
                    }
                }
            }
    ...
    public function sendmail()
    {
        ...
        //Gruppen selected
        if (!is_array($_POST['groupid']) || 'all' == $_POST['groupid'][0]) {
            $_POST['groupid'] = ['all'];
        }
        ...
        if ($_POST['send']) {
            ...
            //Sektionen auflisten
            $grouplist = '<option value="all"'.iif('all' == $_POST['groupid'][0], 'selected="selected"').' style="font-weight:bold;">'.$apx->lang->get('ALL').'</option>';
            $data = $db->fetch('SELECT groupid,name FROM '.PRE.'_user_groups ORDER BY name ASC');
            if (count($data)) {
                foreach ($data as $res) {
    ...
    public function sendmail_exec()
    {
        ...
        if (!isset($_REQUEST['done'])) {
            tmessage('sending', ['FORWARDER' => 'action.php?action=user.sendmail&amp;doit=1&amp;sectoken='.$apx->session->get('sectoken').'&amp;done=0']);
        ...
        if (count($data)) {
            ...
            tmessage('sending', ['FORWARDER' => 'action.php?action=user.sendmail&amp;doit=1&amp;sectoken='.$apx->session->get('sectoken').'&amp;done='.($done + $countPerCall)]);
    ...
    public function sendmail_send($email, $username, $subject, $text)
    {
        global $set,$db,$apx;
    ...
    public function sendpm_exec()
    {
        ...
        //FORWARDER
        if (!isset($_REQUEST['done'])) {
            tmessage('sending', ['FORWARDER' => 'action.php?action=user.sendpm&amp;doit=1&amp;sectoken='.$apx->session->get('sectoken').'&amp;done=0']);
        ...
        if (count($data)) {
            ...
            tmessage('sending', ['FORWARDER' => 'action.php?action=user.sendpm&amp;doit=1&amp;sectoken='.$apx->session->get('sectoken').'&amp;done='.($done + $countPerCall)]);
    ...
    //PM verschicken
    public function sendpm_send($res, $subject, $text, $cleartext)
    {
        global $apx, $db, $set;
