...
if ($user->info['userid'] && $_REQUEST['del']) {
    ...
    if ($_POST['del']) {
        ...
        tmessage('delguestbook', ['ID' => $_REQUEST['del']]);
    }
...
elseif (2 == $usegb && $user->info['userid'] != $userid && 1 != $user->info['groupid']) {
    if ($user->info['userid']) {
        list($isbuddy) = $db->first('SELECT friendid FROM '.PRE."_user_friends WHERE userid='".$_REQUEST['id']."' AND friendid='".$user->info['userid']."' LIMIT 1");
    }
    if (!$isbuddy) {
        message($apx->lang->get('MSG_FRIENDSONLY'));
        require 'lib/_end.php';
...
if ($_REQUEST['add']) {
    ...
    if (!$_POST['text'] || ($set['user']['guestbook_req_title'] && !$_POST['title'])) {
        message('back');
    ...
    } elseif ($set['user']['guestbook_entrymaxlen'] && strlen($_POST['text']) > $set['user']['guestbook_entrymaxlen']) {
        message($apx->lang->get('MSG_TOOLONG'), 'back');
    } elseif (($spam + $set['user']['guestbook_spamprot'] * 60) > time()) {
        message($apx->lang->get('MSG_BLOCKSPAM', ['SEC' => ($spam + $set['user']['guestbook_spamprot'] * 60) - time()]), 'back');
    } else {
