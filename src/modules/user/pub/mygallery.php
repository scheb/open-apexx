...
if ( !$set['user']['gallery'] ) die('function disabled!');
...
if ( $_REQUEST['galid'] ) {
	...
	if ( $_REQUEST['do']=='add' ) {
		if ( $_POST['send'] ) {
			...
			elseif ( $set['user']['gallery_maxpics'] && $piccount+count($files)>$set['user']['gallery_maxpics'] ) message($apx->lang->get('MSG_PICLIMIT',array('LIMIT'=>$set['user']['gallery_maxpics'])),'back');
			else {
			...
			if ( $set['user']['gallery_maxpics'] ) {
				...
				$percent=(round(($set['user']['gallery_maxpics']-$piccount)/iif($set['user']['gallery_maxpics']==0,1,$set['user']['gallery_maxpics']),2)*100).'%';
				$width=(100-$percent).'%';
			...
			if ( $set['user']['gallery_maxpics'] && $piccount>=$set['user']['gallery_maxpics'] ) {
				message($apx->lang->get('MSG_PICLIMITREACHED',array('LIMIT'=>$set['user']['gallery_maxpics'])),'back');
