...
function articles_choose($catid=0,$type=false) {
	...
	if ( !in_array($type,array(false,'normal','preview','review')) ) $type=false;
...
function articles_reviews($sortway=2,$count=5,$start=0,$catid=false,$template='bestreviews') {
	...
	$catid=(int)$catid;
	$sortway=(int)$sortway;
	if ( $sortway!=1 && $sortway!=2 ) $sortway=2; //Standardmäßige Sortierrichtung
	...
	$data=$db->fetch("SELECT ".$fields." FROM ".PRE."_articles AS a LEFT JOIN ".PRE."_user AS b USING(userid) LEFT JOIN ".PRE."_articles_reviews AS c ON a.id=c.artid WHERE ( ".time()." BETWEEN starttime AND endtime AND type='review' ".iif(count($cattree),' AND catid IN ('.@implode(',',$cattree).') ' )." ".section_filter()." ) ORDER BY CAST(c.final_rate AS DECIMAL(4,1) ) ".iif($sortway==1,'ASC','DESC')." LIMIT ".iif($start,$start.',').$count);
...
function articles_print($data,$template,$bestreviews=false) {
	...
	if ( !$bestreviews ) $data=articles_extend_data($data,$parse); 
