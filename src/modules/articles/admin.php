...
class action extends articles_functions
{
    ...
    public function show()
    {
        ...
        if (($_REQUEST['item'] && ($_REQUEST['title'] || $_REQUEST['subtitle'] || $_REQUEST['pages'] || $_REQUEST['teaser'])) || $_REQUEST['secid'] || $_REQUEST['catid'] || $_REQUEST['userid']) {
            ...
            $ids = get_ids($data, 'id');
            $ids[] = -1;
            $searchid = saveSearchResult('admin_articles', $ids, [
                'item' => $_REQUEST['item'],
        ...
        $layerdef[] = ['LAYER_ALL', 'action.php?action=articles.show', !$_REQUEST['what']];
        $layerdef[] = ['NORMALS', 'action.php?action=articles.show&amp;what=type&amp;type=normal', 'type' == $_REQUEST['what'] && 'normal' == $_REQUEST['type']];
        $layerdef[] = ['PREVIEWS', 'action.php?action=articles.show&amp;what=type&amp;type=preview', 'type' == $_REQUEST['what'] && 'preview' == $_REQUEST['type']];
        $layerdef[] = ['REVIEWS', 'action.php?action=articles.show&amp;what=type&amp;type=review', 'type' == $_REQUEST['what'] && 'review' == $_REQUEST['type']];
        $layerdef[] = ['LAYER_SELF', 'action.php?action=articles.show&amp;what=self', 'self' == $_REQUEST['what']];
        ...
        $orderdef['user'] = ['b.username', 'ASC', 'COL_USER'];
        $orderdef['category'] = ['catname', 'ASC', 'COL_CATEGORY'];
        ...
        pages('action.php?action=articles.show&amp;what='.$_REQUEST['what'].iif('type' == $_REQUEST['what'], '&amp;type='.$_REQUEST['type']).iif($_REQUEST['searchid'], '&amp;searchid='.$_REQUEST['searchid']).'&amp;sortby='.$_REQUEST['sortby'], $count);
        ...
        orderstr($orderdef, 'action.php?action=articles.show&amp;what='.$_REQUEST['what'].iif('type' == $_REQUEST['what'], '&amp;type='.$_REQUEST['type']).iif($_REQUEST['searchid'], '&amp;searchid='.$_REQUEST['searchid']));
    ...
    public function show_print($data)
    {
        ...
        $col[] = ['', 1, 'align="center"'];
        $col[] = ['', 1, 'align="center"'];
        $col[] = [$apx->lang->get('COL_TITLE').' / '.$apx->lang->get('COL_USER'), 50, 'class="title"'];
        $col[] = ['COL_CATEGORY', 22, 'align="center"'];
        $col[] = ['COL_PUBDATE', 18, 'align="center"'];
        ...
        if (count($data)) {
            foreach ($data as $res) {
                ...
                if ($res['username']) {
                    $tabledata[$i]['COL3'] .= '<br /><small>'.$apx->lang->get('BY').' '.replace($res['username']).'</small>';
                } else {
                    $tabledata[$i]['COL3'] .= '<br /><small>'.$apx->lang->get('BY').' '.$apx->lang->get('GUEST').': <i>'.replace($res['send_username']).'</i></small>';
                }

                if ($res['starttime']) {
                    $tabledata[$i]['COL5'] = mkdate($res['starttime'], '<br />');
                } else {
                    $tabledata[$i]['COL5'] = '&nbsp;';
                }
                ...
                //Optionen
                if ($apx->user->has_right('articles.edit') && ($res['userid'] == $apx->user->info['userid'] || $apx->user->has_spright('articles.edit'))) {
                    $tabledata[$i]['OPTIONS'] .= optionHTML('edit.gif', 'articles.edit', 'id='.$res['id'], $apx->lang->get('CORE_EDIT'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($apx->user->has_right('articles.copy') && ($res['userid'] == $apx->user->info['userid'] || $apx->user->has_spright('articles.copy'))) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('copy.gif', 'articles.copy', 'id='.$res['id'], $apx->lang->get('COPY'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($apx->user->has_right('articles.del') && ($res['userid'] == $apx->user->info['userid'] || $apx->user->has_spright('articles.del'))) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('del.gif', 'articles.del', 'id='.$res['id'], $apx->lang->get('CORE_DEL'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($pages && (!$res['starttime'] || $res['endtime'] < time()) && $apx->user->has_right('articles.enable') && ($res['userid'] == $apx->user->info['userid'] || $apx->user->has_spright('articles.enable'))) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('enable.gif', 'articles.enable', 'id='.$res['id'], $apx->lang->get('CORE_ENABLE'));
                } elseif ($res['starttime'] && $apx->user->has_right('articles.disable') && ($res['userid'] == $apx->user->info['userid'] || $apx->user->has_spright('articles.disable'))) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('disable.gif', 'articles.disable', 'id='.$res['id'], $apx->lang->get('CORE_DISABLE'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }
                ...
                if ($apx->is_module('comments')) {
                    ...
                    if ($comments && ($apx->is_module('comments') && $set['articles']['coms']) && $res['allowcoms'] && $apx->user->has_right('comments.show')) {
                        $tabledata[$i]['OPTIONS'] .= optionHTML('comments.gif', 'comments.show', 'module=articles&mid='.$res['id'], $apx->lang->get('COMMENTS').' ('.$comments.')');
                    } else {
                        $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                    }
                }
                if ($apx->is_module('ratings')) {
                    ...
                    if ($ratings && ($apx->is_module('ratings') && $set['articles']['ratings']) && $res['allowrating'] && $apx->user->has_right('ratings.show')) {
                        $tabledata[$i]['OPTIONS'] .= optionHTML('ratings.gif', 'ratings.show', 'module=articles&mid='.$res['id'], $apx->lang->get('RATINGS').' ('.$ratings.')');
                    } else {
                        $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                    }
                }
                ...
                //Spacer
                if (isset($laststicky) && (int) $res['sticky'] != $laststicky) {
                    $tabledata[$i]['SPACER'] = true;
                }
    ...
    public function add()
    {
        ...
        if (!in_array($_REQUEST['pageid'], ['new', 'conclusion', 'pics'])) {
        ...
        if (1 == $_POST['send']) {
            if (!checkToken()) {
                infoInvalidToken();
            } elseif (!$_POST['type'] || !$_POST['title'] || !$_POST['catid']) {
                infoNotComplete();
            } elseif ('newcat' != $_POST['catid'] && !$this->category_is_open($_POST['catid'])) {
                die('you are not allowed to add articles to this category!');
            } elseif (!$this->update_artpic()) { /*DO NOTHING*/
            } else {
                //Autor
                if (!$apx->user->has_spright('article.edit')) {
                ...
                //Sticky Ende
                if ($_POST['sticky'] && 0 != ($stickyend = maketime(3))) {
                ...
                if (in_array($_POST['type'], ['preview', 'review'])) {
                ...
                if ('newcat' == $_POST['catid'] && $apx->user->has_right('articles.catadd')) {
                    printJSRedirect('action.php?action=articles.catadd&addid='.$nid.'&pubnow='.intval($_POST['pubnow']).'&from=add');
                } else {
                    printJSRedirect('action.php?action=articles.add&id='.$nid.'&pageid=new&pubnow='.intval($_POST['pubnow']));
                }
            }
        } else {
            $_POST['type'] = 'normal';
            ...
            while (count($linklist) < 20) {
                $linklist[] = ['TITLE' => $apx->lang->get('LLINK'), 'POPUP' => 1];
            }
    ...
    public function del()
    {
        ...
        if (1 == $_POST['send']) {
            if (!checkToken()) {
                ...
                if ($apx->is_module('comments')) {
                    $db->query('DELETE FROM '.PRE."_comments WHERE ( module='articles' AND mid='".$_REQUEST['id']."' )");
                }
                if ($apx->is_module('ratings')) {
                    $db->query('DELETE FROM '.PRE."_ratings WHERE ( module='articles' AND mid='".$_REQUEST['id']."' )");
                }
            ...
            $apx->tmpl->assign('MESSAGE', $apx->lang->get('MSG_TEXT', ['TITLE' => compatible_hsc($title.($subtitle ? ' - '.$subtitle : ''))]));
    ...
    public function copy()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            list($title, $subtitle) = $db->first('SELECT title, subtitle FROM '.PRE."_articles WHERE id='".$_REQUEST['id']."' LIMIT 1");
            $apx->tmpl->assign('MESSAGE', $apx->lang->get('MSG_TEXT', ['TITLE' => compatible_hsc($title.($subtitle ? ' - '.$subtitle : ''))]));
            tmessageOverlay('copy', ['ID' => $_REQUEST['id']]);
        }
    }
    ...
    public function enable()
    {
        ...
        if (1 == $_POST['send']) {
            if (!checkToken()) {
                ...
                if (!$check) {
                    die('can not enable article! no pages created.');
                }
    ...
    public function pagecontent()
    {
        ...
        if (!$this->access_pages($_REQUEST['id'])) {
            die('you have no right to access this article!');
        }
        ...
        if (in_array($this->type, ['preview', 'review']) && 'conclusion' == $_REQUEST['pageid']) {
            $apx->lang->dropaction('articles', 'conclusion');
            if ('preview' == $this->type) {
                $this->conclusion_preview();
            } else {
                $this->conclusion_review();
            }
        }

        //Bilderserie
        elseif ('pics' == $_REQUEST['pageid']) {
    ...
    public function page_index()
    {
        ...
        echo'<h2>'.$apx->lang->get(iif('normal' == $this->type, 'ARTICLE', strtoupper($this->type))).': '.$this->title.'</h2>';
    ...
    public function page_add_edit()
    {
        ...
        if ($_REQUEST['move'] && $_REQUEST['direction']) {
            $this->page_move();
        ...
        if (1 == $_POST['send']) {
            ...
            $apx->tmpl->assign('ID', $_REQUEST['id']);
            $apx->tmpl->assign('ACTION', iif('articles.add' == $_REQUEST['action'], 'add', 'edit'));
    ...
    public function page_del()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            tmessageOverlay('pdel', ['ID' => $_REQUEST['id'], 'PAGEID' => $_REQUEST['pageid'], 'DEL' => $_REQUEST['del'], 'PUBNOW' => $_REQUEST['pubnow']]);
        }
    }
    ...
    //***************************** Fazit: Vorschau *****************************
    public function conclusion_preview()
    {
        global $set,$db,$apx;
        ...
        if ($_POST['send']) {
            ...
            if (!checkToken()) {
                infoInvalidToken();
            } elseif (!$_POST['impression'] || !$_POST['conclusion'] || $custom_failed) {
                infoNotComplete();
            } else {
    ...
    //***************************** Fazit: Tests *****************************
    public function conclusion_review()
    {
        global $set,$db,$apx;
        ...
        if ($_POST['send']) {
            ...
            if (!checkToken()) {
                infoInvalidToken();
            } elseif (!$_POST['conclusion'] || !$_POST['final_rate'] || $custom_failed || $rating_failed) {
                infoNotComplete();
            } else {
    ...
    public function pictures()
    {
        ...
        //Bild löschen
        if (isset($_REQUEST['delpic'], $pictures[$_REQUEST['delpic']])) {
            if ($_POST['delpic']) {
                if (!checkToken()) {
                    printInvalidToken();
                } else {
                ...
                tmessageOverlay('picdel', ['ID' => $_REQUEST['id'], 'DELPIC' => $_REQUEST['delpic']]);
            }
        }
        ...
        elseif ($_POST['send']) {
            ...
            for ($i = 1; $i <= 5; ++$i) {
                ...
                //Bild skalieren
                if (false !== $picture && !$_POST['noresize'.$i] && $set['articles']['picwidth'] && $set['articles']['picheight']) {
                    $scaled = $img->resize(
                        $picture,
                ...
                if (false !== $picture && $set['articles']['watermark'] && $_POST['watermark'.$i]) {
    ...
    public function finish_article()
    {
        global $set,$db,$apx;
        if ('articles.add' == $_REQUEST['action']) {
            //Veröffentlichung
    ...
    public function catshow()
    {
        ...
        $col[] = ['COL_ARTICLES', 25, 'align="center"'];

        if ($set['articles']['subcats']) {
            $data = $this->cat->getTree(['title', 'open']);
        } else {
            $orderdef[0] = 'title';
            $orderdef['title'] = ['title', 'ASC', 'COL_CATNAME'];
    ...
    public function catedit()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            if (!checkToken()) {
                ...
                infoNotComplete();
            } elseif (!$_POST['open'] && $articles) {
                info($apx->lang->get('INFO_CONTAINSARTICLES'));
            } else {
                if ('all' == $_POST['groupid'][0]) {
                    $_POST['forgroup'] = 'all';
                } else {
            ...
            $res = $this->cat->getNode($_REQUEST['id'], ['title', 'icon', 'open', 'forgroup']);
    ...
    public function catdel()
    {
        ...
        list($articles) = $db->first('SELECT count(id) FROM '.PRE."_articles WHERE catid='".$_REQUEST['id']."'");
        if ($articles) {
            die('category still contains articles!');
        }
    ...
    public function catclean()
    {
        ...
        if ($set['articles']['subcats']) {
            ...
            $data = $db->fetch('SELECT id,title,open FROM '.PRE.'_articles_cat ORDER BY title ASC');
        }
        if (count($data)) {
            foreach ($data as $res) {
                if ($res['level']) {
