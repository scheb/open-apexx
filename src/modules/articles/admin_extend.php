...
function access_pages($id) {
	global $set,$db,$apx;
	$id=(int)$id;
	if ( !$id ) die('page access: missing artid!');
	
	if ( $apx->user->has_spright('articles.edit') ) return true;
...
function get_brothers() {
	...
	if ( is_int($_REQUEST['pageid']) ) list($thisord)=$db->first("SELECT ord FROM ".PRE."_articles_pages WHERE ( artid='".$_REQUEST['id']."' AND id='".$_REQUEST['pageid']."' ) LIMIT 1");
	else $thisord=9999999;
	if ( $thisord ) {
	...
	return array((int)$brother1,(int)$brother2);
}
...
function get_catlist($selected=null) {
	...
	if ( $set['articles']['subcats'] ) $data = $this->cat->getTree(array('title', 'open', 'forgroup'));
	else $data=$db->fetch("SELECT id,title,open,forgroup FROM ".PRE."_articles_cat ORDER BY title ASC");
...
function update_artpic() {
	...
	if ( $_POST['delpic'] ) $this->del_artpic();
...
function upload_pic() {
	global $set,$db,$apx;
	if ( !is_uploaded_file($_FILES['pic_upload']['tmp_name']) ) die('artpic has not been uploaded!');
...
function process_pic($tempfile,$filename) {
	...
	if ( $width && $height ) $pic=$this->img->resize($source,$width,$height,$set['articles']['artpic_quality'],0);
	else $pic=$source;
	...
	if ( !$set['articles']['artpic_popup'] || !$set['articles']['artpic_width'] || !$set['articles']['artpic_height'] ) {
		if ( $picfile!=$this->artpicpath ) $this->del_artpic();
	...
	if ( $thumbfile!=$this->artpicpath ) $this->del_artpic();
