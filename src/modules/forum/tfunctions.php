...
function forum_threads_new($count = 5, $inforumid = 0, $notforumid = 0, $template = 'new')
{
    ...
    if (count($forumids)) {
        $data = $db->fetch('SELECT '.$fields.' FROM '.PRE.'_forum_threads WHERE ( del=0 AND moved=0 AND forumid IN ('.implode(',', $forumids).') ) ORDER BY opentime DESC '.iif($count, 'LIMIT '.$count));
    } else {
        $data = [];
    }
...
function forum_stats($template = 'stats')
{
    ...
    if (in_template(['COUNT_THREADS', 'AVG_HITS'], $parse)) {
