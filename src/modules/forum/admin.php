...
class action extends forum_functions
{
    ...
    public $rightfields = [
        'right_visible' => ['all'],
        'right_read' => ['all'],
        'right_open' => ['public', 'indiv'],
        'right_announce' => ['none'],
        'right_post' => ['public', 'indiv'],
        'right_editpost' => ['public', 'indiv'],
        'right_delpost' => ['public', 'indiv'],
        'right_delthread' => ['none'],
        'right_addattachment' => ['public', 'indiv'],
        'right_readattachment' => ['all'],
    ];
    ...
    public function show()
    {
        ...
        $col[] = ['COL_THREADS', 15, 'align="center"'];
        $col[] = ['COL_POSTS', 15, 'align="center"'];

        //Foren auslesen
        $data = $this->cat->getTree(['iscat', 'title', 'posts', 'threads']);
    ...
    //Forum erstellen
    public function add_forum($ftype, $parentinfo = [])
    {
        global $set,$db,$apx;
        ...
        foreach ($this->rightfields as $index => $info) {
            if (!is_array($_POST[$index])) {
                $_POST[$index] = [];
            } elseif ('all' == $_POST[$index][0]) {
                $_POST[$index] = ['all'];
            } elseif ('none' == $_POST[$index][0]) {
                $_POST[$index] = ['none'];
            }
        }

        //Moderator-Variable zu Array
        if (!is_array($_POST['moderator'])) {
            $_POST['moderator'] = [];
        }
        ...
        if (1 == $_POST['send']) {
            if (!checkToken()) {
                ...
                if (!$_POST['inherit']) {
                    foreach ($this->rightfields as $index => $trash) {
                        $rightfields .= ','.$index;
                        if ('all' == $_POST[$index][0]) {
                            $_POST[$index] = 'all';
                        } elseif ('none' == $_POST[$index][0]) {
                            $_POST[$index] = '';
                        } else {
                            $_POST[$index] = forum_serialize($_POST[$index]);
                        }
                    }
                }
                ...
                foreach ($this->rightfields as $index => $info) {
                    foreach ($data as $res) {
                        if (in_array('all', $info)) {
                            $_POST[$index] = ['all'];
                        } elseif (in_array('none', $info)) {
                            $_POST[$index] = ['none'];
                        } elseif (in_array($res['gtype'], $info)) {
                            $_POST[$index][] = $res['groupid'];
                        }
                    }
                }
            }
            ...
            if ('forum' == $ftype) {
                $apx->tmpl->parse('add_edit_forum');
            } else {
                $apx->tmpl->parse('add_edit_category');
            }
        }
    }
    ...
    public function edit()
    {
        ...
        if ($_POST['id']) {
            ...
            $fields = 'iscat,title,description,meta_description,stylesheet,link,moderator,password,open,searchable,countposts,inherit'.$rightfields;
        }

        $info = $this->cat->getNode($_REQUEST['id'], explode(',', $fields));
        if ($info['iscat']) {
            $this->edit_forum('cat', $info);
        } else {
            $this->edit_forum('forum', $info);
        }
    }

    //Forum bearbeiten
    public function edit_forum($ftype, $foruminfo = [])
    {
        global $set,$db,$apx;
        ...
        if (1 == $_POST['send']) {
            ...
            //Rechte
            foreach ($this->rightfields as $index => $info) {
                if ('all' == $foruminfo[$index]) {
                    $_POST[$index] = ['all'];
                } elseif (!$foruminfo[$index]) {
                    $_POST[$index] = ['none'];
                } else {
                    $_POST[$index] = forum_unserialize($foruminfo[$index]);
                }
            }
    ...
    public function clean()
    {
        ...
        if (1 == $_POST['send'] && $_POST['moveto'] >= 0) {
            if (!checkToken()) {
                printInvalidToken();
            } else {
            ...
            //Foren auflisten
            $data = $this->cat->getTree(['title', 'iscat', 'link']);
            if (count($data)) {
                foreach ($data as $res) {
                    if ($res['level'] > 1) {
                        $space = str_repeat('&nbsp;&nbsp;', $res['level'] - 1);
    ...
    public function del()
    {
        ...
        $finfo = $this->cat->getNode($_REQUEST['id'], ['posts', 'parents', 'children']);
        if ($finfo['posts']) {
            die('forum still contains postings!');
        }
        if ($finfo['children']) {
            die('forum still has children!');
        }
    ...
    public function announce()
    {
        ...
        $col[] = ['COL_USER', 30, 'align="center"'];
        $col[] = ['COL_PUBDATE', 20, 'align="center"'];

        $orderdef[0] = 'addtime';
        $orderdef['title'] = ['a.title', 'ASC', 'COL_TITLE'];
        $orderdef['username'] = ['b.username', 'ASC', 'COL_USER'];
        $orderdef['addtime'] = ['a.starttime', 'DESC', 'SORT_ADDTIME'];
        $orderdef['publication'] = ['a.starttime', 'DESC', 'COL_PUBDATE'];
    ...
    public function announce_add()
    {
        ...
        //Forum-Liste
        if (!is_array($_POST['forumid']) || 'all' == $_POST['forumid'][0]) {
            $_POST['forumid'] = ['all'];
        }
    ...
    public function announce_del()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            $apx->tmpl->assign('MESSAGE', $apx->lang->get('MSG_TEXT', ['TITLE' => compatible_hsc($title)]));
            tmessageOverlay('announcedel', ['ID' => $_REQUEST['id']]);
        }
    }
    ...
    public function ranks()
    {
        ...
        //Layer
        $layerdef[] = ['LAYER_POST', 'action.php?action=forum.ranks&amp;what=posts', 'posts' == $_REQUEST['what']];
        $layerdef[] = ['LAYER_USER', 'action.php?action=forum.ranks&amp;what=user', 'user' == $_REQUEST['what']];
        $layerdef[] = ['LAYER_USERGROUPS', 'action.php?action=forum.ranks&amp;what=groups', 'groups' == $_REQUEST['what']];

        //Layer Header ausgeben
        $html->layer_header($layerdef);

        ///////////////////// Benutzer-Ränge
        if ('user' == $_REQUEST['what']) {
            $col[] = ['COL_RANK', 50, 'class="title"'];
            $col[] = ['COL_USER', 50, 'align="center"'];

            $orderdef[0] = 'title';
            $orderdef['title'] = ['a.title', 'ASC', 'COL_RANK'];
            $orderdef['username'] = ['b.username', 'ASC', 'COL_USER'];
        ...
        elseif ('groups' == $_REQUEST['what']) {
            $col[] = ['COL_RANK', 70, 'class="title"'];
            $col[] = ['COL_USERGROUP', 30, 'align="center"'];

            $orderdef[0] = 'title';
            $orderdef['title'] = ['a.title', 'ASC', 'COL_RANK'];
            $orderdef['group'] = ['b.name', 'ASC', 'COL_USERGROUP'];
        ...
        ///////////////////// Posting-Ränge
            ...
            $col[] = ['COL_RANK', 70, 'class="title"'];
            $col[] = ['COL_POSTS', 30, 'align="center"'];

            $orderdef[0] = 'posts';
            $orderdef['title'] = ['title', 'ASC', 'COL_RANK'];
            $orderdef['posts'] = ['minposts', 'ASC', 'COL_POSTS'];
    ...
    public function ranks_add()
    {
        ...
        if ($_POST['send']) {
            ...
            if (!checkToken()) {
                infoInvalidToken();
            } elseif (!$_POST['title'] || ('posts' == $_POST['criteria'] && ('' === $_POST['minposts'] || intval($_POST['minposts']) < 0)) || ('user' == $_POST['criteria'] && !$_POST['username']) || ('groups' == $_POST['criteria'] && !$_POST['groupid'])) {
                infoNotComplete();
            } elseif ('user' == $_POST['criteria'] && !$userid) {
                info($apx->lang->get('INFO_USERFAILED'));
            } else {
                if ('posts' == $_POST['criteria']) {
                    unset($_POST['username'],$_POST['groupid']);
                }
                if ('groups' == $_POST['criteria']) {
    ...
    public function ranks_del()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            $apx->tmpl->assign('MESSAGE', $apx->lang->get('MSG_TEXT', ['TITLE' => compatible_hsc($title)]));
            tmessageOverlay('rankdel', ['ID' => $_REQUEST['id']]);
        }
    }
    ...
    public function filetypes()
    {
        ...
        $col[] = ['ICON', 10, 'align="center"'];
        $col[] = ['EXT', 45, 'align="center"'];
        $col[] = ['MAXSIZE', 45, 'align="center"'];
    ...
    public function filetypes_edit()
    {
        global $set,$apx,$db;
        if (!$_REQUEST['id']) {
            die('missing ID!');
    ...
    public function filetypes_del()
    {
        ...
        if ($_POST['send']) {
            ...
            $apx->tmpl->assign('MESSAGE', $apx->lang->get('MSG_TEXT', ['TITLE' => compatible_hsc($_REQUEST['id'])]));
            tmessageOverlay('filetypedel', ['ID' => $_REQUEST['id']]);
        }
    }
    ...
    public function icons()
    {
        ...
        //Aktionen
        if ('add' == $_REQUEST['do']) {
            return $this->icons_add();
        }
        if ('del' == $_REQUEST['do']) {
            return $this->icons_del();
        }
    ...
    public function icons_del()
    {
        ...
        if ($_POST['send']) {
            ...
            tmessageOverlay('icondel', ['ID' => $_REQUEST['id']]);
        }
    }
        ...
        if ($_REQUEST['send']) {
            if (!checkToken()) {
                ...
                if (count($data)) {
                    ...
                    tmessage('reindex_forwarder', ['FORWARDER' => 'action.php?action=forum.reindex&amp;send=1&amp;limit='.($_REQUEST['limit'] + $step).'&sectoken='.$apx->session->get('sectoken')]);
                }
    ...
    public function update_index($text, $threadid, $postid, $title = false)
    {
        global $set,$db,$apx;
