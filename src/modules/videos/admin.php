...
class action extends videos_functions
{
    ...
    public function show()
    {
        ...
        if (($_REQUEST['item'] && ($_REQUEST['title'] || $_REQUEST['text'])) || $_REQUEST['secid'] || $_REQUEST['catid'] || $_REQUEST['userid']) {
            ...
            $ids = get_ids($data, 'id');
            $ids[] = -1;
            $searchid = saveSearchResult('admin_videos', $ids, [
                'item' => $_REQUEST['item'],
        ...
        $layerdef[] = ['LAYER_ALL', 'action.php?action=videos.show', !$_REQUEST['what']];
        $layerdef[] = ['LAYER_BROKEN', 'action.php?action=videos.show&amp;what=broken', 'broken' == $_REQUEST['what']];
        if ($set['videos']['ffmpeg'] && $set['videos']['flvtool2']) {
            $layerdef[] = ['LAYER_FAILED', 'action.php?action=videos.show&amp;what=failed', 'failed' == $_REQUEST['what']];
        }
        ...
        $orderdef['user'] = ['b.username', 'ASC', 'COL_AUTHOR'];
        ...
        $orderdef['downloads'] = ['a.downloads', 'DESC', 'COL_DOWNLOADS'];
    ...
    public function show_print($data)
    {
        ...
        $col[] = ['', 1, 'align="center"'];
        $col[] = [$apx->lang->get('COL_TITLE').' / '.$apx->lang->get('COL_AUTHOR'), 35, 'class="title"'];
        ...
        if ($apx->user->has_right('videos.del')) {
            $multiactions[] = [$apx->lang->get('CORE_DEL'), 'action.php?action=videos.del', false];
        }
        if ($apx->user->has_right('videos.enable')) {
            $multiactions[] = [$apx->lang->get('CORE_ENABLE'), 'action.php?action=videos.enable', false];
        }
        if ($apx->user->has_right('videos.disable')) {
            $multiactions[] = [$apx->lang->get('CORE_DISABLE'), 'action.php?action=videos.disable', false];
        }
    ...
    public function add()
    {
        ...
        if (1 == $_POST['send']) {
            if (!checkToken()) {
                ...
                infoNotComplete();
            } elseif ('embed' == $_POST['source'] && !$_POST['embed_url']) {
                infoNotComplete();
            } elseif ('select' == $_POST['source'] && !$_POST['select_flv']) {
                infoNotComplete();
            } elseif ('external' == $_POST['source'] && (!$_POST['external_flv'] || ($_POST['external_file'] && !$_POST['external_filesize']))) {
                infoNotComplete();
            } elseif ('convert' == $_POST['source'] && !$_POST['convert_file']) {
                infoNotComplete();
            } elseif ('embed' == $_POST['source'] && !($extInfo = $this->getEmbedVideo($_POST['embed_url']))) {
                info($apx->lang->get('INFO_EMBED_NOTFOUND'));
            } elseif ('select' == $_POST['source'] && !file_exists(BASEDIR.getpath('uploads').$_POST['select_flv'])) {
                info($apx->lang->get('INFO_NOTEXISTS', ['FILE' => $_POST['select_flv']]));
            } elseif ('select' == $_POST['source'] && $_POST['select_file'] && !file_exists(BASEDIR.getpath('uploads').$_POST['select_file'])) {
                info($apx->lang->get('INFO_NOTEXISTS', ['FILE' => $_POST['select_file']]));
            } elseif ('convert' == $_POST['source'] && !file_exists(BASEDIR.getpath('uploads').$_POST['convert_file'])) {
                info($apx->lang->get('INFO_NOTEXISTS', ['FILE' => $_POST['convert_file']]));
                ...
                elseif ('external' == $vSource) {
                    ...
                    if ($_POST['external_file']) {
                        ...
                        if ('kb' == $_POST['external_filesize_format']) {
                            $_POST['filesize'] = (int) 1024 * (float) str_replace(',', '.', $_POST['external_filesize']);
                        } elseif ('mb' == $_POST['external_filesize_format']) {
                            $_POST['filesize'] = (int) 1024 * 1024 * (float) str_replace(',', '.', $_POST['external_filesize']);
                        } elseif ('gb' == $_POST['external_filesize_format']) {
                            $_POST['filesize'] = (int) 1024 * 1024 * 1024 * (float) str_replace(',', '.', $_POST['external_filesize']);
                        } else {
                ...
                elseif ('select' == $vSource && $_POST['select_screens']) {
    ...
    public function edit()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            if (!checkToken()) {
                ...
                infoNotComplete();
            } elseif ('apexx' == $source && !$_POST['select_flv']) {
                infoNotComplete();
            } elseif ('apexx' != $source && $_POST['embed_url'] && !($extInfo = $this->getEmbedVideo($_POST['embed_url']))) {
                info($apx->lang->get('INFO_EMBED_NOTFOUND'));
            } elseif ('apexx' == $source && !file_exists(BASEDIR.getpath('uploads').$_POST['select_flv'])) {
                info($apx->lang->get('INFO_NOTEXISTS', ['FILE' => $_POST['select_flv']]));
            } elseif ('apexx' == $source && $_POST['select_file'] && !file_exists(BASEDIR.getpath('uploads').$_POST['select_file'])) {
                info($apx->lang->get('INFO_NOTEXISTS', ['FILE' => $_POST['select_file']]));
            } elseif ('external' == $source && (!$_POST['external_flv'] || ($_POST['external_file'] && !$_POST['external_filesize']))) {
                infoNotComplete();
            } elseif (!$this->category_is_open($_POST['catid'])) {
                die('you are not allowed to add videos to this category!');
            } elseif (!$this->update_teaserpic()) { /*DO NOTHING*/
		...
		");
            ...
            if ('apexx' == $_POST['source'] && $_POST['file']) {
                $apx->tmpl->assign('PASSWORD', compatible_hsc($_POST['password']));
                $apx->tmpl->assign('REGONLY', $_POST['regonly']);
                $apx->tmpl->assign('LIMIT', $_POST['limit']);
            } elseif ('external' == $_POST['source'] && $_POST['file']) {
                $apx->tmpl->assign('PASSWORD', compatible_hsc($_POST['password']));
                $apx->tmpl->assign('REGONLY', $_POST['regonly']);
                $apx->tmpl->assign('LIMIT', $_POST['limit']);
            ...
            if (!checkToken()) {
                ...
                foreach ($data as $res) {
                    ...
                    if ($poppic && file_exists(BASEDIR.getpath('uploads').$poppic)) {
                        $this->mm->deletefile($poppic);
                    }

                    //Dateien löschen
                    if ('apexx' == $source && $_POST['delfile']) {
                        if ($file1 && file_exists(BASEDIR.getpath('uploads').$file1)) {
                            $this->mm->deletefile($file1);
                        }
                        if ($file2 && file_exists(BASEDIR.getpath('uploads').$file2)) {
                            $this->mm->deletefile($file2);
                        }
                    }
                    ...
                    if ($picdata) {
                        foreach ($picdata as $pic) {
                            if ($pic['picture'] && file_exists(BASEDIR.getpath('uploads').$pic['picture'])) {
                                $this->mm->deletefile($pic['picture']);
                            }
                            if ($pic['thumbnail'] && file_exists(BASEDIR.getpath('uploads').$pic['thumbnail'])) {
                                $this->mm->deletefile($pic['thumbnail']);
                            }
                        }
                    }
                    ...
                    if ($apx->is_module('comments')) {
                        $db->query('DELETE FROM '.PRE."_comments WHERE ( module='videos' AND mid='".$res['id']."' )");
                    }
                    if ($apx->is_module('ratings')) {
                        $db->query('DELETE FROM '.PRE."_ratings WHERE ( module='videos' AND mid='".$res['id']."' )");
                    }
        ...
        //Einzeln
            ...
            if (1 == $_POST['send']) {
                if (!checkToken()) {
                    ...
                    //Kommentare und Bewertungen
                    if ($apx->is_module('comments')) {
                        $db->query('DELETE FROM '.PRE."_comments WHERE ( module='videos' AND mid='".$_REQUEST['id']."' )");
                    }
                    if ($apx->is_module('ratings')) {
                        $db->query('DELETE FROM '.PRE."_ratings WHERE ( module='videos' AND mid='".$_REQUEST['id']."' )");
                    }
    ...
    public function convert()
    {
        ...
        if ($_REQUEST['password'] != $set['main']['crypt']) {
        ...
        if ('apexx' == $res['source'] && ('new' == $res['status'] || 'failed' == $res['status'])) {
    ...
    public function pshow()
    {
        ...
        if ($this->access_pics($_REQUEST['id'], 'videos.padd')) {
            quicklink('videos.padd', 'action.php', 'id='.$_REQUEST['id']);
        }
        ...
        if (is_array($pictures) && count($pictures)) {
            foreach ($pictures as $res) {
                ...
                //Optionen
                if ($apx->user->has_right('videos.pdel') && $this->access_pics($_REQUEST['id'], 'videos.pdel')) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('del.gif', 'videos.pdel', 'id='.$res['pictureid'].'&dlid='.$_REQUEST['id'], $apx->lang->get('CORE_DEL'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }
            }
        ...
        $apx->tmpl->assign('TABLE', $tabledata);
        $multiactions = [];
        if ($this->access_pics($_REQUEST['id'], 'videos.pdel')) {
            $multiactions[] = [$apx->lang->get('CORE_DEL'), 'action.php?action=videos.pdel&id='.$_REQUEST['id']];
        }
        $html->table($col, $multiactions);
    }
    ...
    public function padd()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            for ($i = 1; $i <= $set['videos']['addpics']; ++$i) {
                ...
                //Bild skalieren
                if (false !== $picture && !$_POST['noresize'.$i] && $set['videos']['picwidth'] && $set['videos']['picheight']) {
                    $scaled = $img->resize(
                        $picture,
                        $set['videos']['picwidth'],
                ...
                if (false !== $picture && $set['videos']['watermark'] && $_POST['watermark'.$i]) {
    ...
    public function pdel()
    {
        ...
        //Einzeln
            ...
            if (!$this->access_pics($_REQUEST['dlid'], 'videos.pdel')) {
                die('you have no right to access this video!');
            }
    ...
    public function catshow()
    {
        ...
        $col[] = ['COL_VIDEOS', 25, 'align="center"'];
    ...
    public function catdel()
    {
        ...
        list($videos) = $db->first('SELECT count(id) FROM '.PRE."_videos WHERE catid='".$_REQUEST['id']."'");
        if ($videos) {
            die('category still contains videos!');
        }
