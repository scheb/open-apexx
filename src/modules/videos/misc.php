...
function misc_videofile()
{
    ...
    if ($res['password'] && $_POST['password'] != $res['password']) {
        tmessage('pwdrequired', ['ID' => $_REQUEST['id'], 'SECHASH' => $_REQUEST['sechash']], 'videos');
    }
    ...
    //Datei downloadbar?
    if (!in_array($res['source'], ['apexx', 'external'])) {
    ...
    videos_insert_stats($res['id'], $thefsize, 'apexx' == $res['source']);

    //Datei senden
    if ('external' == $res['source']) {
        header('HTTP/1.1 301 Moved Permanently');
