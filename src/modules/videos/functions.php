...
function videos_embedcode($source, $id) {
	...
	if ( isset($plattforms[$source]) ) {
		return strtr($plattforms[$source][3], array(
			'{VIDEOID}' => $id,
...
function videos_filesize($info) {
	if ( $info['source']=='external' ) return $info['filesize'];
	elseif ( $info['source']=='apexx' && file_exists(BASEDIR.getpath('uploads').$info['file']) ) return filesize(BASEDIR.getpath('uploads').$info['file']);
	return 0;
}
...
function videos_catinfo($id=false) {
	...
	//Alle Kategorien;
		...
		else $data=$db->fetch("SELECT * FROM ".PRE."_videos_cat ORDER BY title ASC");
		if ( !count($data) ) return array();
		foreach ( $data AS $res ) {
			$catinfo[$res['id']]=$res;
			$catinfo[$res['id']]['link']=mklink(
...
function videos_insert_stats($id,$filesize=0,$local=true) {
	global $set,$db,$apx,$user;
	$id=(int)$id;
	if ( !$set['videos']['exttraffic'] && !$local ) $filesize=0;
	...
	if ( $stats_exists ) $db->query("UPDATE ".PRE."_videos_stats SET hits=hits+1 WHERE ( daystamp='".$datestamp."' AND dlid='".$id."' )");
	else {
		$db->quieterror=true;
...
function videos_limit_is_reached($id=false,$limit=false) {
	...
	if ( $set['videos']['maxtraffic'] ) {
		...
		if ( $traffic>=$set['videos']['maxtraffic'] ) return true;
	}
...
function videos_showcomments($id) {
	...
	if ( !$set['videos']['coms'] || !$res['allowcoms'] || !$apx->is_module('comments') ) return;
