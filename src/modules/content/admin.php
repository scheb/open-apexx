...
class action
{
    ...
    public function show()
    {
        ...
        if (($_REQUEST['item'] && ($_REQUEST['title'] || $_REQUEST['text'])) || $_REQUEST['secid'] || $_REQUEST['catid'] || $_REQUEST['userid']) {
            ...
            $ids = get_ids($data, 'id');
            $ids[] = -1;
            $searchid = saveSearchResult('admin_content', $ids, [
        ...
        $orderdef['time'] = ['a.time', 'DESC', 'COL_ADDTIME'];
        $orderdef['lastchange'] = ['a.lastchange', 'DESC', 'COL_LASTCHANGE'];
        $orderdef['hits'] = ['a.hits', 'DESC', 'COL_HITS'];

        $col[] = ['', 1, 'align="center"'];
        $col[] = ['COL_TITLE', 50, 'class="title"'];
        $col[] = ['COL_USER', 20, 'align="center"'];
        $col[] = ['COL_LASTCHANGE', 20, 'align="center"'];
    ...
    public function del()
    {
        ...
        if (1 == $_POST['send']) {
            if (!checkToken()) {
                ...
                if ($db->affected_rows()) {
                    if ($apx->is_module('comments')) {
                        $db->query('DELETE FROM '.PRE."_comments WHERE ( module='content' AND mid='".$_REQUEST['id']."' )");
                    }
                    if ($apx->is_module('ratings')) {
                        $db->query('DELETE FROM '.PRE."_ratings WHERE ( module='content' AND mid='".$_REQUEST['id']."' )");
                    }
                }
    ...
    public function group()
    {
        ...
        //Kategorie leeren
        if ('clean' == $_REQUEST['do'] && isset($data[$_REQUEST['id']])) {
        ...
        $col[] = ['COL_CONTENTS', 20, 'align="center"'];
