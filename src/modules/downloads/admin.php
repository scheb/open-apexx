...
class action extends downloads_functions
{
    ...
    public function show()
    {
        ...
        if (($_REQUEST['item'] && ($_REQUEST['title'] || $_REQUEST['text'])) || $_REQUEST['secid'] || $_REQUEST['catid'] || $_REQUEST['userid']) {
            ...
            $ids = get_ids($data, 'id');
            $ids[] = -1;
            $searchid = saveSearchResult('admin_downloads', $ids, [
                'item' => $_REQUEST['item'],
        ...
        $layerdef[] = ['LAYER_ALL', 'action.php?action=downloads.show', !$_REQUEST['what']];
        $layerdef[] = ['LAYER_SEND', 'action.php?action=downloads.show&amp;what=send', 'send' == $_REQUEST['what']];
        $layerdef[] = ['LAYER_BROKEN', 'action.php?action=downloads.show&amp;what=broken', 'broken' == $_REQUEST['what']];
    ...
    public function show_print($data)
    {
        ...
        $col[] = ['', 1, 'align="center"'];
        $col[] = [$apx->lang->get('COL_TITLE').' / '.$apx->lang->get('COL_UPLOADER'), 45, 'class="title"'];
        $col[] = ['COL_CATEGORY', 25, 'align="center"'];
        $col[] = ['COL_ADDTIME', 20, 'align="center"'];
        $col[] = ['COL_DOWNLOADS', 10, 'align="center"'];
        ...
        if (count($data)) {
            foreach ($data as $res) {
                ...
                //Optionen
                if ($apx->user->has_right('downloads.edit') && ($res['userid'] == $apx->user->info['userid'] || $apx->user->has_spright('downloads.edit'))) {
                    $tabledata[$i]['OPTIONS'] .= optionHTML('edit.gif', 'downloads.edit', 'id='.$res['id'], $apx->lang->get('CORE_EDIT'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($apx->user->has_right('downloads.del') && ($res['userid'] == $apx->user->info['userid'] || $apx->user->has_spright('downloads.del'))) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('del.gif', 'downloads.del', 'id='.$res['id'], $apx->lang->get('CORE_DEL'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ((!$res['starttime'] || $res['endtime'] < time()) && $apx->user->has_right('downloads.enable') && ($res['userid'] == $apx->user->info['userid'] || $apx->user->has_spright('downloads.enable'))) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('enable.gif', 'downloads.enable', 'id='.$res['id'], $apx->lang->get('CORE_ENABLE'));
                } elseif ($res['starttime'] && $apx->user->has_right('downloads.disable') && ($res['userid'] == $apx->user->info['userid'] || $apx->user->has_spright('downloads.disable'))) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('disable.gif', 'downloads.disable', 'id='.$res['id'], $apx->lang->get('CORE_DISABLE'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }
                ...
                if ($apx->user->has_right('downloads.pshow')) {
                    $tabledata[$i]['OPTIONS'] .= optionHTML('pic.gif', 'downloads.pshow', 'id='.$res['id'], $apx->lang->get('PICS'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }
                ...
                if ($apx->is_module('comments')) {
                    ...
                    if ($comments && ($apx->is_module('comments') && $set['downloads']['coms']) && $res['allowcoms'] && $apx->user->has_right('comments.show')) {
                        $tabledata[$i]['OPTIONS'] .= optionHTML('comments.gif', 'comments.show', 'module=downloads&mid='.$res['id'], $apx->lang->get('COMMENTS').' ('.$comments.')');
                    } else {
                        $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                    }
                }
                if ($apx->is_module('ratings')) {
                    ...
                    if ($ratings && ($apx->is_module('ratings') && $set['downloads']['ratings']) && $res['allowrating'] && $apx->user->has_right('ratings.show')) {
                        $tabledata[$i]['OPTIONS'] .= optionHTML('ratings.gif', 'ratings.show', 'module=downloads&mid='.$res['id'], $apx->lang->get('RATINGS').' ('.$ratings.')');
                    } else {
                        $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                    }
                }
        ...
        $multiactions = [];
        if ($apx->user->has_right('downloads.del')) {
            $multiactions[] = [$apx->lang->get('CORE_DEL'), 'action.php?action=downloads.del', false];
        }
        if ($apx->user->has_right('downloads.enable')) {
            $multiactions[] = [$apx->lang->get('CORE_ENABLE'), 'action.php?action=downloads.enable', false];
        }
        if ($apx->user->has_right('downloads.disable')) {
            $multiactions[] = [$apx->lang->get('CORE_DISABLE'), 'action.php?action=downloads.disable', false];
        }
    ...
    public function add()
    {
        ...
        if (1 == $_POST['send']) {
            if (!checkToken()) {
                infoInvalidToken();
            } elseif (1 == $_FILES['file_upload']['error']) {
                ...
                if ($_POST['local']) {
                    ...
                    if ('kb' == $_POST['filesize_format']) {
                        $_POST['filesize'] = (int) 1024 * (float) str_replace(',', '.', $_POST['filesize']);
                    } elseif ('mb' == $_POST['filesize_format']) {
                        $_POST['filesize'] = (int) 1024 * 1024 * (float) str_replace(',', '.', $_POST['filesize']);
                    } elseif ('gb' == $_POST['filesize_format']) {
                        $_POST['filesize'] = (int) 1024 * 1024 * 1024 * (float) str_replace(',', '.', $_POST['filesize']);
                    }
                }
                ...
                if ($_POST['submit_padd']) {
                    printJSRedirect('action.php?action=downloads.padd&id='.$nid);
                } else {
                    printJSRedirect('action.php?action=downloads.show');
                }
            }
        } else {
            $_POST['local'] = 1;
    ...
    public function edit_enable($tempfile, $filename)
    {
        global $set,$db,$apx;
        ...
        if (is_array($_REQUEST['multiid'])) {
            if (!checkToken()) {
                ...
                foreach ($data as $res) {
                    ...
                    if ($res['file'] && file_exists(BASEDIR.getpath('uploads').$res['file'])) {
                        $this->mm->deletefile($res['file']);
                    }
                    if ($res['tempfile'] && file_exists(BASEDIR.getpath('uploads').$res['tempfile'])) {
                        $this->mm->deletefile($res['tempfile']);
                    }
                    if ($apx->is_module('comments')) {
                        $db->query('DELETE FROM '.PRE."_comments WHERE ( module='downloads' AND mid='".$res['id']."' )");
                    }
                    if ($apx->is_module('ratings')) {
                        $db->query('DELETE FROM '.PRE."_ratings WHERE ( module='downloads' AND mid='".$res['id']."' )");
                    }
        ...
        //Einzeln
            ...
            if (1 == $_POST['send']) {
                if (!checkToken()) {
                    ...
                    //Dateien löschen
                    if ($_POST['delfile']) {
                        if ($file && file_exists(BASEDIR.getpath('uploads').$file)) {
                            $this->mm->deletefile($file);
                        }
                        if ($tempfile && file_exists(BASEDIR.getpath('uploads').$tempfile)) {
                    ...
                    if ($apx->is_module('comments')) {
                        $db->query('DELETE FROM '.PRE."_comments WHERE ( module='downloads' AND mid='".$_REQUEST['id']."' )");
                    }
                    if ($apx->is_module('ratings')) {
                        $db->query('DELETE FROM '.PRE."_ratings WHERE ( module='downloads' AND mid='".$_REQUEST['id']."' )");
                    }
    ...
    public function enable()
    {
        ...
        //Einzeln
            ...
            if ($check) {
                die('can not enable download! missing file!');
            }
    ...
    public function pshow()
    {
        ...
        if ($this->access_pics($_REQUEST['id'], 'downloads.padd')) {
            quicklink('downloads.padd', 'action.php', 'id='.$_REQUEST['id']);
        }

        $col[] = ['COL_THUMBNAIL', 100, 'align="center"'];
        ...
        if (is_array($pictures) && count($pictures)) {
            foreach ($pictures as $key => $res) {
                ...
                //Optionen
                if ($apx->user->has_right('downloads.pdel') && $this->access_pics($_REQUEST['id'], 'downloads.pdel')) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('del.gif', 'downloads.pdel', 'id='.$key.'&dlid='.$_REQUEST['id'], $apx->lang->get('CORE_DEL'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }
            }
        ...
        $apx->tmpl->assign('TABLE', $tabledata);
        $multiactions = [];
        if ($this->access_pics($_REQUEST['id'], 'downloads.pdel')) {
            $multiactions[] = [$apx->lang->get('CORE_DEL'), 'action.php?action=downloads.pdel&id='.$_REQUEST['id']];
        }
        $html->table($col, $multiactions);
    }
    ...
    public function padd()
    {
        ...
        if (!$this->access_pics($_REQUEST['id'], 'downloads.padd')) {
            die('you have no right to access this download!');
        }
        ...
        if (1 == $_POST['send']) {
            ...
            for ($i = 1; $i <= $set['downloads']['addpics']; ++$i) {
                ...
                if (!in_array($imginfo[2], [IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG])) {
                    $error[] = $apx->lang->get('MSG_NOIMAGE', ['NAME' => $_FILES['upload'.$i]['name']]);
                }
                if ($error) {
            ...
            for ($i = 1; $i <= $set['downloads']['addpics']; ++$i) {
                ...
                if (!$this->mm->uploadfile($_FILES['upload'.$i], 'downloads/pics', $newname)) {
                    continue;
                }
                //Bild einlesen
                ...
                //Bild skalieren
                if (false !== $picture && !$_POST['noresize'.$i] && $set['downloads']['picwidth'] && $set['downloads']['picheight']) {
                    $scaled = $img->resize(
                        $picture,
                ...
                if (false !== $picture && $set['downloads']['watermark'] && $_POST['watermark'.$i]) {
    ...
    public function pdel()
    {
        ...
        if (is_array($_REQUEST['multiid'])) {
            if (!checkToken()) {
                ...
                if (count($cache)) {
                    ...
                    $picturelist = unserialize($picturelist);
                    if (!is_array($picturelist)) {
                        die('picturelist is not an array!');
                    }
        ...
        //Einzeln
            ...
            if (!$this->access_pics($_REQUEST['dlid'], 'downloads.pdel')) {
                die('you have no right to access this download!');
            }
            ...
            if (1 == $_POST['send']) {
                ...
                tmessageOverlay('pdel', ['ID' => $_REQUEST['id'], 'DLID' => $_REQUEST['dlid']]);
            }
        }
    }
    ...
    public function catshow()
    {
        ...
        $col[] = ['COL_DOWNLOADS', 25, 'align="center"'];
        ...
        if (count($data)) {
            ...
            foreach ($data as $res) {
                ...
                //Optionen
                if ($apx->user->has_right('downloads.catedit')) {
                    $tabledata[$i]['OPTIONS'] .= optionHTML('edit.gif', 'downloads.catedit', 'id='.$res['id'], $apx->lang->get('CORE_EDIT'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($apx->user->has_right('downloads.catdel') && !$downloads) {
                    $tabledata[$i]['OPTIONS'] .= '<span class="ifhasnochildren">'.optionHTMLOverlay('del.gif', 'downloads.catdel', 'id='.$res['id'], $apx->lang->get('CORE_DEL')).'</span><span class="ifhaschildren"><img alt="" src="design/ispace.gif"/></span>';
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($apx->user->has_right('downloads.catclean') && $downloads) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('clean.gif', 'downloads.catclean', 'id='.$res['id'], $apx->lang->get('CLEAN'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }
    ...
    public function catadd()
    {
        ...
        if (!count($_POST['groupid']) || 'all' == $_POST['groupid'][0]) {
            $_POST['groupid'] = ['all'];
        }
        ...
        if (1 == $_POST['send']) {
            ...
            $grouplist .= '<option value="all"'.iif(!isset($_POST['groupid']) || 'all' == $_POST['groupid'][0], ' selected="selected"').' style="font-weight:bold;">'.$apx->lang->get('ALL').'</option>';
            if (count($data)) {
                foreach ($data as $res) {
    ...
    public function catedit()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            if (!checkToken()) {
                ...
                infoNotComplete();
            } elseif (!$_POST['open'] && $downloads) {
                info($apx->lang->get('INFO_CONTAINSDOWNLOADS'));
            } else {
                if ('all' == $_POST['groupid'][0]) {
                    $_POST['forgroup'] = 'all';
                } else {
            ...
            $res = $this->cat->getNode($_REQUEST['id'], ['title', 'text', 'icon', 'open', 'forgroup']);
    ...
    public function catdel()
    {
        ...
        list($downloads) = $db->first('SELECT count(id) FROM '.PRE."_downloads WHERE catid='".$_REQUEST['id']."'");
        if ($downloads) {
            die('category still contains downloads!');
        }
        ...
        if (count($data)) {
            foreach ($data as $res) {
                ...
                $popdata[$i]['LINK'] = mklink(
                    'downloads.php?id='.$res['id'],
                    'downloads,id'.$res['id'].urlformat($res['title']).'.html',
                    iif($set['main']['forcesection'], iif(unserialize_section($res['secid']) == ['all'], $apx->section_default, array_shift($tmp = unserialize_section($res['secid']))), 0)
                );
            }
        }
