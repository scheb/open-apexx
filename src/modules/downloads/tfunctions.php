...
function downloads_similar($tagids = [], $count = 5, $start = 0, $catid = false, $template = 'similar')
{
    global $set,$db,$apx,$user;
    $count = (int) $count;
    $start = (int) $start;
...
function downloads_print($data, $template)
{
    ...
    //User auslesen
    $userinfo = [];
    if (in_template(['DOWNLOAD.UPLOADER', 'DOWNLOAD.UPLOADER_EMAIL', 'DOWNLOAD.UPLOADER_EMAIL_ENCRYPTED'], $parse)) {
    ...
    if (count($data)) {
        foreach ($data as $res) {
            ...
            if ($apx->is_module('comments') && $set['downloads']['coms'] && $res['allowcoms']) {
                ...
                if (in_template(['DOWNLOAD.COMMENT_LAST_USERID', 'DOWNLOAD.COMMENT_LAST_NAME', 'DOWNLOAD.COMMENT_LAST_TIME'], $parse)) {
...
function downloads_stats($template = 'stats')
{
    ...
    if (in_template(['COUNT_DOWNLOADS', 'AVG_HITS'], $parse)) {
