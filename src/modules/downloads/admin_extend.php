...
function get_userlist() {
	...
	if ( $info['send_username'] ) {
		$userlist='<option value="send"'.iif($_POST['userid']=='send',' selected="selected"').'>'.$apx->lang->get('GUEST').': '.replace($info['send_username']).iif($info['send_email'],' ('.replace($info['send_email']).')').'</option><option value=""></option>';
	}
...
function update_file() {
	...
	if ( $_FILES['file_upload']['tmp_name'] ) return $this->upload_file();
	elseif ( $_POST['file'] ) return $this->link_file();
...
function upload_file() {
	global $set,$db,$apx;
	if ( !is_uploaded_file($_FILES['file_upload']['tmp_name']) ) die('file has not been uploaded!');
...
function get_uploaded_name($base) {
...
function process_pic($tempfile,$filename) {
	...
	if ( $width && $height ) $pic=$this->img->resize($source,$width,$height,$set['downloads']['teaserpic_quality'],0);
	else $pic=$source;
