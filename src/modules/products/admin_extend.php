...
class products_functions
{
    ...
    public function generate_release($ele)
    {
        if ($ele['month'] && $ele['year']) {
            unset($ele['quater']);
        }
        if ($ele['quater']) {
            unset($ele['day'],$ele['month']);
        }
        ...
        if (isset($ele['system'])) {
            return [$reldata, $relstamp, $ele['system']];
        }

        return [$reldata, $relstamp];
    }

    //Einheiten-Liste
    public function get_units($select = 0, $type = 'all')
    {
        global $set,$db,$apx;
        static $cache;
        $select = (int) $select;
    ...
    //Genre-Liste
    public function get_genre($type, $select = 0)
    {
        global $set,$db,$apx;
        $select = (int) $select;
    ...
    //Medien-Liste
    public function get_media($type, $select = [])
    {
        global $set,$db,$apx;
        if (!is_array($select)) {
    ...
    //Systeme auflisten
    public function get_systems($select)
    {
        global $set,$db,$apx;
        if (!is_array($select)) {
            $select = [];
    ...
    //Release-Liste
    public function get_release($type)
    {
        global $set,$db,$apx;
        $release = [];
        ...
        for ($i = 1; $i <= 10; ++$i) {
            ...
            if (!$info['quater'] && !$info['day'] && !$info['month'] && !$info['year']) {
                continue;
            }
            if ('game' == $type) {
                $release[$i]['SYSTEM'] = $this->get_systems([$info['system']]);
            } else {
        ...
        while (count($release) < 10) {
            ++$i;
            $release[$i]['DISPLAY'] = iif(0 == count($release), 1, 0);
            if ('game' == $type) {
                $release[$i]['SYSTEM'] = $this->get_systems([]);
            } else {
    ...
    public function process_pic($tempfile, $filename)
    {
        ...
        if ($width && $height) {
            $pic = $this->img->resize($source, $width, $height, $set['products']['pic_quality'], 0);
        } else {
            $pic = $source;
        }
    ...
    public function update_teaserpic()
    {
        ...
        if ($_POST['delteaserpic']) {
            $this->del_teaserpic();
        }
        if ($_FILES['teaserpic_upload']['tmp_name']) {
            return $this->upload_teaserpic();
        }
        if ($_POST['teaserpic_copy']) {
            return $this->copy_teaserpic();
        }
    ...
    //Produkt-Bild hochladen
    public function upload_teaserpic()
    {
        global $set,$db,$apx;
        if (!is_uploaded_file($_FILES['teaserpic_upload']['tmp_name'])) {
    ...
    //Produkt-Bild kopieren
    public function copy_teaserpic()
    {
        global $set,$db,$apx;
        if (!file_exists(BASEDIR.getpath('uploads').$_POST['teaserpic_copy'])) {
            die('source-file does not exist!');
        }
    ...
    //Bild verarbeiten
    public function process_teaserpic($tempfile, $filename)
    {
        global $set,$db,$apx;
        ...
        if ($width && $height) {
            $teaserpic = $this->img->resize($source, $width, $height, $set['products']['teaserpic_quality'], 0);
        } else {
            $teaserpic = $source;
        }
        ...
        if (!$set['products']['teaserpic_popup'] || !$set['products']['teaserpic_width'] || !$set['products']['teaserpic_height']) {
            if ($teaserpicfile != $this->teaserpicpath) {
                $this->del_teaserpic();
            }
    ...
    public function update_unitpic()
    {
        global $set,$db,$apx;
        ...
        if ($_FILES['pic_upload']['tmp_name']) {
            return $this->upload_unitpic();
        }
        if ($_POST['pic_copy']) {
            return $this->copy_unitpic();
        }
    ...
    //Produkt-Bild hochladen
    public function upload_unitpic()
    {
        global $set,$db,$apx;
        if (!is_uploaded_file($_FILES['pic_upload']['tmp_name'])) {
            die('pic has not been uploaded!');
    ...
    //Bild verarbeiten
    public function process_unitpic($tempfile, $filename)
    {
        global $set,$db,$apx;
