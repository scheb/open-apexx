...
class comments
{
    ...
    //Startup
    public function comments($module, $mid = false)
    {
        global $set,$apx;
    ...
    public function getsettings($module)
    {
        ...
        foreach ($set['comments'] as $key => $info) {
            if (isset($set[$module]['com_'.$key])) {
                $this->set[$key] = $set[$module]['com_'.$key];
            } else {
                $this->set[$key] = $set['comments'][$key];
            }
        }
    ...
    public function assign_comments($parse = false)
    {
        ...
        if (false === $parse || in_array('COMMENT', $parse)) {
            $apx->tmpl->assign('COMMENT', $this->display());
        }
        if (false === $parse || in_array('COMMENT_COUNT', $parse)) {
            $apx->tmpl->assign('COMMENT_COUNT', $this->count());
        }
        if (false === $parse || in_array('COMMENT_LINK', $parse)) {
            $apx->tmpl->assign('COMMENT_LINK', $this->link());
        }
        if (false === $parse || in_array('COMMENT_POSTTO', $parse)) {
            $apx->tmpl->assign('COMMENT_POSTTO', $this->postto());
        }
        if ($this->set['captcha'] && !$user->info['userid'] && (false === $parse || in_array('COMMENT_CAPCHA', $parse) || in_array('COMMENT_CAPTCHA', $parse))) {
    ...
    // Kommentar-Link
    public function link($page = false, $varname = 'comments')
    {
        global $db,$apx,$user;
    ...
    public function get_lastcomment()
    {
        global $db,$apx,$user;
        if (isset($this->lastinfo[$this->mid])) {
            return;
        }
    ...
    public function last_userid($page = false)
    {
        global $db,$apx,$user;
    ...
    public function last_name($page = false)
    {
        global $db,$apx,$user;
        $this->get_lastcomment();
    ...
    public function last_time($page = false)
    {
        global $db,$apx,$user;
        $this->get_lastcomment();
    ...
    public function ip_is_blocked()
    {
        global $set;

        if (!is_array($set['comments']['blockip'])) {
            $set['comments']['blockip'] = [];
        }
        foreach ($set['comments']['blockip'] as $res) {
            if ((false === $res['endip'] && $res['startip'] == ip2float(get_remoteaddr())) || (ip2float(get_remoteaddr()) >= $res['startip'] && ip2float(get_remoteaddr()) <= $res['endip'])) {
                return true;
            }
        }
    ...
    public function text_is_blocked()
    {
        global $set;
        $text = $_POST['text'];

        if (!is_array($set['comments']['blockstring'])) {
            $set['comments']['blockstring'] = [];
        }
        foreach ($set['comments']['blockstring'] as $string) {
            $string = trim($string);
            $string = preg_quote($string);
            $string = str_replace('\\*', '(.*)', $string);
            if (preg_match('#'.$string.'#miU', $text)) {
                return true;
            }
        }
    ...
    public function addcom()
    {
        ...
        if ($user->info['userid']) {
            if ($captchafailed) {
                ...
                message($apx->lang->get('MSG_COM_BLOCKSPAM', ['SEC' => ($spam + $this->set['spamprot'] * 60) - time()]), 'back');
            } else {
