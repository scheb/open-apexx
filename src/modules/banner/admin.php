...
function show() {
	...
	if ( count($data) ) {
		foreach ( $data AS $res ) {
			...
			//Limit erreicht?
			if ( $res['limit'] && $res['views']>=$res['limit'] ) $tabledata[$i]['COL4']='<span style="color:red;">'.$tabledata[$i]['COL4'].'</span>';
			
			//Optionen
			if ( $apx->user->has_right('banner.edit') ) $tabledata[$i]['OPTIONS'].=optionHTML('edit.gif', 'banner.edit', 'id='.$res['id'], $apx->lang->get('CORE_EDIT'));
			else $tabledata[$i]['OPTIONS'].='<img src="design/ispace.gif" alt="" />';
			
			if ( $apx->user->has_right('banner.del') ) $tabledata[$i]['OPTIONS'].=optionHTMLOverlay('del.gif', 'banner.del', 'id='.$res['id'], $apx->lang->get('CORE_DEL'));
			else $tabledata[$i]['OPTIONS'].='<img src="design/ispace.gif" alt="" />';
			
			if ( ( !$res['starttime'] || $res['endtime']<time() ) && $apx->user->has_right('banner.enable') ) $tabledata[$i]['OPTIONS'].=optionHTMLOverlay('enable.gif', 'banner.enable', 'id='.$res['id'], $apx->lang->get('CORE_ENABLE'));
			elseif ( $res['starttime'] && $apx->user->has_right('banner.disable') ) $tabledata[$i]['OPTIONS'].=optionHTMLOverlay('disable.gif', 'banner.disable', 'id='.$res['id'], $apx->lang->get('CORE_DISABLE'));
			else $tabledata[$i]['OPTIONS'].='<img src="design/ispace.gif" alt="" />';
		}
	}
...
function add() {
	...
	if ( $_POST['send']==1 ) {
		...
		elseif ( !$_POST['partner'] || !$_POST['code'] || !$_POST['ratio'] ) infoNotComplete(); 
		else {
			...
			if ( (int)$_POST['ratio']<1 ) $_POST['ratio']=1;
...
global $set,$apx,$tmpl,$db,$user;
	...
	if ( $_POST['send']==1 ) {
		...
		elseif ( !$_POST['partner'] || !$_POST['code'] || !$_POST['ratio'] ) infoNotComplete(); 
			...
			if ( intval($_POST['ratio'])<1 ) $_POST['ratio']=1;
...
function enable() {
	...
	if ( $_POST['send']==1 ) {
		...
		if ( !checkToken() ) return printInvalidToken();
		elseif ( $starttime ) {
...
function group() {
	...
	foreach ( $data AS $id => $res ) {
		...
		if ( !$count ) $tabledata[$i]['OPTIONS'].=optionHTMLOverlay('del.gif', 'banner.group', 'do=del&id='.$id, $apx->lang->get('CORE_DEL'));
		else $tabledata[$i]['OPTIONS'].='<img src="design/ispace.gif" alt="" />';
	}
	$apx->tmpl->assign('TABLE',$tabledata);
	$html->table($col);
