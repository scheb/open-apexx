...
class EpiOAuth
{
    ...
    protected function curlInit($url)
    {
        ...
        if (isset($_SERVER['SERVER_ADDR']) && !empty($_SERVER['SERVER_ADDR']) && '127.0.0.1' != $_SERVER['SERVER_ADDR']) {
    ...
    protected function normalizeUrl($url = null)
    {
        ...
        if (!empty($port) && (('http' === $scheme && 80 != $port) || ('https' === $scheme && 443 != $port))) {
            $retval .= ":{$port}";
        }
