...
class action
{
    ...
    public function areParentsEnabled($id)
    {
        $path = $this->cat->getPathTo(['starttime'], $id);
        foreach ($path as $res) {
            if ($res['id'] == $id) {
                break;
            }
    ...
    public function show()
    {
        ...
        $col[] = ['', 1, ''];
        $col[] = ['COL_QUESTION', 80, 'class="title"'];
        $col[] = ['COL_HITS', 20, 'align="center"'];
    ...
    public function add()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            //Baum
            $catlist = '<option value="root" style="font-weight:bold;"'.iif('root' == $_POST['parent'], ' selected="selected"').'>'.$apx->lang->get('ROOT').'</option><option value=""></option>';
            $data = $this->cat->getTree(['question']);
            if (count($data)) {
                foreach ($data as $res) {
    ...
    public function edit()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            $res = $this->cat->getNode($_REQUEST['id'], ['question', 'meta_description', 'answer', 'searchable']);
            if (!$res['parents']) {
                $_POST['parent'] = 'root';
            } else {
