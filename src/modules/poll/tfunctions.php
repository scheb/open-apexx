...
function poll_small($id = false, $template = 'poll')
{
    ...
    if (
        ...
        || '1' == $_COOKIE[$set['main']['cookie_pre'].'_voted'][$pollinfo['id']]
        || $ipblock
        ...
        if ('1' == $_COOKIE[$set['main']['cookie_pre'].'_voted'][$pollinfo['id']] || $ipblock) {
            $set_voted = 1;
        }
...
//Ähnliche Umfragen
function poll_similar($tagids = [], $count = 5, $start = 0, $template = 'similar')
{
    global $set,$db,$apx,$user;
    $count = (int) $count;
    $start = (int) $start;
    ...
    if (count($data)) {
        foreach ($data as $res) {
            ...
            if ($apx->is_module('comments') && $set['poll']['coms'] && $res['allowcoms']) {
                ...
                if (in_template(['POLL.COMMENT_LAST_USERID', 'POLL.COMMENT_LAST_NAME', 'POLL.COMMENT_LAST_TIME'], $parse)) {
...
function poll_stats($template = 'stats')
{
    ...
    if (in_template(['COUNT_POLLS', 'AVG_VOTES'], $parse)) {
