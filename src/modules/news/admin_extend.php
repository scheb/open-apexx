...
class news_functions
{
    ...
    public function update_newspic()
    {
        global $set,$db,$apx;
    ...
    public function upload_pic()
    {
        ...
        if (!is_uploaded_file($_FILES['pic_upload']['tmp_name'])) {
            die('newspic has not been uploaded!');
        }
    ...
    public function process_pic($tempfile, $filename)
    {
        ...
        if ($width && $height) {
            $pic = $this->img->resize($source, $width, $height, $set['news']['newspic_quality'], 0);
        } else {
            $pic = $source;
        }
