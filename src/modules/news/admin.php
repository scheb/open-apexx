...
class action extends news_functions
{
    ...
    public function show()
    {
        ...
        if (($_REQUEST['item'] && ($_REQUEST['title'] || $_REQUEST['subtitle'] || $_REQUEST['text'] || $_REQUEST['teaser'])) || $_REQUEST['secid'] || $_REQUEST['catid'] || $_REQUEST['userid']) {
            ...
            $ids = get_ids($data, 'id');
            $ids[] = -1;
            $searchid = saveSearchResult('admin_news', $ids, [
                'item' => $_REQUEST['item'],
        ...
        $layerdef[] = ['LAYER_ALL', 'action.php?action=news.show', !$_REQUEST['what']];
        $layerdef[] = ['LAYER_SELF', 'action.php?action=news.show&amp;what=self', 'self' == $_REQUEST['what']];
        $layerdef[] = ['LAYER_SEND', 'action.php?action=news.show&amp;what=send', 'send' == $_REQUEST['what']];
    ...
    public function show_print($data)
    {
        ...
        if (count($data)) {
            foreach ($data as $res) {
                ...
                //Optionen
                if ($apx->user->has_right('news.edit') && ($res['userid'] == $apx->user->info['userid'] || $apx->user->has_spright('news.edit'))) {
                    $tabledata[$i]['OPTIONS'] .= optionHTML('edit.gif', 'news.edit', 'id='.$res['id'], $apx->lang->get('CORE_EDIT'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($apx->user->has_right('news.copy') && ($res['userid'] == $apx->user->info['userid'] || $apx->user->has_spright('news.copy'))) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('copy.gif', 'news.copy', 'id='.$res['id'], $apx->lang->get('COPY'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($apx->user->has_right('news.del') && ($res['userid'] == $apx->user->info['userid'] || $apx->user->has_spright('news.del'))) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('del.gif', 'news.del', 'id='.$res['id'], $apx->lang->get('CORE_DEL'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ((!$res['starttime'] || $res['endtime'] < time()) && $apx->user->has_right('news.enable') && ($res['userid'] == $apx->user->info['userid'] || $apx->user->has_spright('news.enable'))) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('enable.gif', 'news.enable', 'id='.$res['id'], $apx->lang->get('CORE_ENABLE'));
                } elseif ($res['starttime'] && $apx->user->has_right('news.disable') && ($res['userid'] == $apx->user->info['userid'] || $apx->user->has_spright('news.disable'))) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('disable.gif', 'news.disable', 'id='.$res['id'], $apx->lang->get('CORE_DISABLE'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }
                ...
                if ($apx->is_module('comments')) {
                    ...
                    if ($comments && ($apx->is_module('comments') && $set['news']['coms']) && $res['allowcoms'] && $apx->user->has_right('comments.show')) {
                        $tabledata[$i]['OPTIONS'] .= optionHTML('comments.gif', 'comments.show', 'module=news&mid='.$res['id'], $apx->lang->get('COMMENTS').' ('.$comments.')');
                    } else {
                        $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                    }
                }
                if ($apx->is_module('ratings')) {
                    ...
                    if ($ratings && ($apx->is_module('ratings') && $set['news']['ratings']) && $res['allowrating'] && $apx->user->has_right('ratings.show')) {
                        $tabledata[$i]['OPTIONS'] .= optionHTML('ratings.gif', 'ratings.show', 'module=news&mid='.$res['id'], $apx->lang->get('RATINGS').' ('.$ratings.')');
                    } else {
                        $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                    }
                }
    ...
    public function add()
    {
        ...
        if (1 == $_POST['send']) {
            if (!checkToken()) {
                ...
                if ('newcat' == $_POST['catid'] && $apx->user->has_right('news.catadd')) {
            ...
            while (count($sourcelist) < 20) {
                $sourcelist[] = ['TITLE' => $apx->lang->get('LSOURCE'), 'POPUP' => 1];
            }
    ...
    public function catedit()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            if (!checkToken()) {
                ...
                infoNotComplete();
            } elseif (!$_POST['open'] && $news) {
                info($apx->lang->get('INFO_CONTAINSNEWS'));
            } else {
                if ('all' == $_POST['groupid'][0]) {
                    $_POST['forgroup'] = 'all';
                } else {
    ...
    public function catdel()
    {
        ...
        list($news) = $db->first('SELECT count(id) FROM '.PRE."_news WHERE catid='".$_REQUEST['id']."'");
        if ($news) {
            die('category still contains news!');
        }
    ...
    public function catclean()
    {
        ...
        if ($set['news']['subcats']) {
            ...
            $data = $db->fetch('SELECT id,title,open FROM '.PRE.'_news_cat ORDER BY title ASC');
        }
        if (count($data)) {
            foreach ($data as $res) {
                if ($res['level']) {
    ...
    public function sshow()
    {
        ...
        $orderdef[0] = 'title';
        $orderdef['title'] = ['title', 'ASC', 'SORT_TITLE'];
