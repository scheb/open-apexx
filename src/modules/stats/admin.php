...
class action
{
    ...
    public function getweekstamp($time)
    {
    ...
    public function visitors()
    {
        ...
        if ($count_today && $sumpercent) {
            $progn = round($count_today / $sumpercent);
        }
        ...
        //LAYER DEFINIEREN
        $layerdef[] = ['LAYER_TABLE', 'action.php?action=stats.visitors', !$_REQUEST['what']];
        $layerdef[] = ['LAYER_GRAPH', 'action.php?action=stats.visitors&amp;what=graph', 'graph' == $_REQUEST['what']];

        $html->layer_header($layerdef);

        if ('graph' == $_REQUEST['what']) {
            $this->visitors_graph($newdata);
        } else {
            $this->visitors_table($progn);
        }
    ...
    //TABELLEN
    public function visitors_table($progn)
    {
        global $set,$db,$apx;
        ...
        if (count($data)) {
            ...
            foreach ($data as $res) {
                ...
                if ($res['daystamp'] == date('Ymd', time() - TIMEDIFF)) {
                    ...
                    if ($prognwidth) {
                        $lastdata[$i]['PROGNOSIS'] = ($prognwidth - $width).'%';
                    }
                }
            }
        }
    ...
    //DIAGRAMME
    public function visitors_graph($newdata)
    {
        global $set,$db,$apx;
    ...
    public function agents()
    {
        ...
        if (count($data)) {
            foreach ($data as $res) {
                ...
                if ('SEARCHENGINE' == $res['browser'] || 'UNKNOWN' == $res['browser']) {
                    $browsername = $apx->lang->get($res['browser']);
                } else {
    ...
    public function os()
    {
        ...
        if (count($data)) {
            foreach ($data as $res) {
                ...
                if ('SEARCHENGINE' == $res['os'] || 'UNKNOWN' == $res['os']) {
                    $osname = $apx->lang->get($res['os']);
                } else {
                    $osname = $res['os'];
                }
    ...
    public function countries()
    {
        ...
        if (count($data)) {
            foreach ($data as $res) {
                ...
                if ('UNKNOWN' == $res['country']) {
                    $cname = $apx->lang->get($res['country']);
                } else {
                    $cname = $res['country'];
                }
    ...
    public function referer()
    {
        ...
        if ($apx->user->has_spright('stats.referer')) {
            ...
            if (isset($_REQUEST['delfilter'])) {
                if ($_POST['send']) {
                    ...
                    tmessageOverlay('filterdel', ['DELFILTER' => $_REQUEST['delfilter']]);
                }
            ...
            $layerdef[] = ['LAYER_STATS', 'action.php?action=stats.referer', !$_REQUEST['what']];
            $layerdef[] = ['LAYER_FILTER', 'action.php?action=stats.referer&amp;what=filter', 'filter' == $_REQUEST['what']];
            $html->layer_header($layerdef);

            //Host-Filter anzeigen
            if ('filter' == $_REQUEST['what']) {
                $filter = $set['stats']['referer_filter'];
                asort($filter);
        ...
        if (count($data)) {
            ...
            foreach ($data as $res) {
                ++$i;
                if (!isset($hits[$res['url']])) {
                    list($hits[$res['url']]) = $db->first('SELECT sum(hits) FROM '.PRE."_stats_referer WHERE ( daystamp>='".date('Ymd', time() - 7 * 24 * 3600 - TIMEDIFF)."' AND hash='".addslashes(md5($res['url']))."' AND url='".addslashes($res['url'])."' )");
                }
