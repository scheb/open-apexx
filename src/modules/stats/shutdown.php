...
if (!defined('NOSTATS') && !$_COOKIE[$set['main']['cookie_pre'].'_stats_count'] && !$check) {
    ...
    if ($exists['browser']) {
        $db->query('UPDATE '.PRE."_stats_userenv SET hits=hits+1 WHERE ( daystamp='".$daystamp."' AND type='browser' AND value='".addslashes($browser)."' ) LIMIT 1");
    } else {
        $db->query('INSERT INTO '.PRE."_stats_userenv VALUES ('".$daystamp."','browser','".addslashes($browser)."',1)");
    }
    if ($exists['os']) {
        $db->query('UPDATE '.PRE."_stats_userenv SET hits=hits+1 WHERE ( daystamp='".$daystamp."' AND type='os' AND value='".addslashes($os)."' ) LIMIT 1");
    } else {
        $db->query('INSERT INTO '.PRE."_stats_userenv VALUES ('".$daystamp."','os','".addslashes($os)."',1)");
    }
    if ($exists['country']) {
        $db->query('UPDATE '.PRE."_stats_userenv SET hits=hits+1 WHERE ( daystamp='".$daystamp."' AND type='country' AND value='".addslashes($country)."' ) LIMIT 1");
    } else {
        $db->query('INSERT INTO '.PRE."_stats_userenv VALUES ('".$daystamp."','country','".addslashes($country)."',1)");
    }

    //Besucher zählen
    if ('SEARCHENGINE' != $browser || $set['stats']['countsearchengine']) {
...
///////////////////////////// Hits /////////////////////////////
elseif (!(false !== strpos($_SERVER['PHP_SELF'], 'misc.php') && 'counter' == $_REQUEST['action']) && $_SERVER['REQUEST_URI']) {
    require_once BASEDIR.getmodulepath('stats').'functions.php';
    $weekstamp = stats_weekstamp($statsnow);
...
///////////////////////////// Referer /////////////////////////////
if (
    'counter' != $_REQUEST['action'] //Zählpixel filtern
    && $_SERVER['HTTP_REFERER']
    && 'http://' == strtolower(substr($_SERVER['HTTP_REFERER'], 0, 7))
    && ($set['stats']['ownreferer'] || false === strpos(strtolower($_SERVER['HTTP_REFERER']), 'http://'.strtolower($_SERVER['HTTP_HOST'])))
) {
    require_once BASEDIR.getmodulepath('stats').'functions.php';
