...
function gallery_choose()
{
    ...
    if ($set['gallery']['subgals']) {
        ...
        $data = $tree->getTree(['title'], null, "'".time()."' BETWEEN starttime AND endtime");
    } else {
...
//Neuste Galerien
function gallery_similar($tagids = [], $count = 5, $start = 0, $template = 'similar')
{
    global $set,$db,$apx,$user;
    $count = (int) $count;
    $start = (int) $start;
...
function gallery_printpic($data, $template)
{
    ...
    if (count($data)) {
        ...
        $galinfo = [];
        if (in_template(['PICTURE.GALLERY_TITLE', 'PICTURE.GALLERY_DESCRIPTION', 'PICTURE.GALLERY_LINK', 'PICTURE.GALLERY_TIME', 'PICTURE.GALLERY_PRODUCT_ID', 'PICTURE.GALLERY_RESTRICTED'], $parse)) {
        ...
        foreach ($data as $res) {
            ...
            //Enthaltene Bilder, Letzte Aktualisierung
            if (in_template(['PICTURE.GALLERY_COUNT', 'PICTURE.GALLERY_UPDATETIME'], $parse)) {
...
function gallery_printsingle($res, $template)
{
    ...
    //GALERIE
    if (in_template(['GALLERY_TITLE', 'GALLERY_DESCRIPTION', 'GALLERY_LINK', 'GALLERY_TIME', 'GALLERY_PRODUCT_ID', 'GALLERY_RESTRICTED'], $parse)) {
    ...
    //Enthaltene Bilder, Letzte Aktualisierung
    if (in_template(['GALLERY_COUNT', 'GALLERY_UPDATETIME'], $parse)) {
        list($galcount, $updatetime) = $db->first('SELECT count(id), max(addtime) FROM '.PRE."_gallery_pics WHERE ( galid='".$res['galid']."' AND active='1' )");
...
function gallery_stats($template = 'stats')
{
    ...
    if (in_template(['COUNT_PICTURES', 'AVG_HITS'], $parse)) {
