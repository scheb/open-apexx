...
class action
{
    ...
    public function setGalleryUpdatetime($galId)
    {
        global $db;
    ...
    public function show()
    {
        ...
        if (!$set['gallery']['subgals'] && $_REQUEST['item']) {
            ...
            $ids = get_ids($data, 'id');
            $ids[] = -1;
            $searchid = saveSearchResult('admin_gallery', $ids, [
                'item' => $_REQUEST['item'],
            ]);
        ...
        $orderdef['starttime'] = ['starttime', 'DESC', 'COL_STARTTIME'];
        ...
        $col[] = ['COL_TITLE', 60, 'class="title"'];
        $col[] = ['COL_STARTTIME', 25, 'align="center"'];
        $col[] = ['COL_COUNT', 15, 'align="center"'];
        ...
        if (!$set['gallery']['subgals']) {
            ...
            $data = $this->cat->getTree(['*'], null, section_filter(false, 'secid'));
        }
    ...
    public function add()
    {
        ...
        if (1 == $_POST['send']) {
            if (!checkToken()) {
                ...
                //WENN NODE
                if ($set['gallery']['subgals'] && 'root' != $_POST['parent']) {
    ...
    public function edit()
    {
        ...
        if (1 == $_POST['send']) {
                ...
                if ($set['gallery']['subgals']) {
                    ...
                    if (intval($_POST['parent'])) {
                        ...
                        if ($currentParentId != $_POST['parent']) {
                            $rootNode = $this->cat->getNode($_POST['parent'], ['secid', 'password', 'restricted']);
            ...
            $res = $this->cat->getNode($_REQUEST['id'], ['secid', 'prodid', 'title', 'description', 'meta_description', 'password', 'starttime', 'endtime', 'searchable', 'restricted', 'allowcoms']);
            ...
            //Baum
            if ($set['gallery']['subgals']) {
                $gallist = '<option value="root" style="font-weight:bold;"'.iif('root' == $_POST['parent'], ' selected="selected"').'>'.$apx->lang->get('ROOT').'</option>';
                $data = $this->cat->getTree(['title']);
                if (count($data)) {
    ...
    public function enable()
    {
        ...
        if (1 == $_POST['send']) {
            if (!checkToken()) {
                ...
                $path = $this->cat->getPathTo(['starttime'], $_REQUEST['id']);
                foreach ($path as $res) {
                    if (!$res['starttime']) {
    ...
    public function move()
    {
        ...
        if (!$set['gallery']['subgals'] && 3 != $set['gallery']['ordergal']) {
            die('moving of galleries is disabled!');
        }
    ...
    public function pshow()
    {
        ...
        $orderdef[0] = 'time';
        $orderdef['time'] = ['id', 'DESC', 'SORT_ADDTIME'];
        ...
        $col[] = ['COL_CAPTION', 70, 'class="title"'];
        ...
        $multiactions = [];
        if ($apx->user->has_right('gallery.pmove')) {
            $multiactions[] = [$apx->lang->get('MOVE'), 'action.php?action=gallery.pmove&gid='.intval($_REQUEST['id']), true];
        }
        if ($apx->user->has_right('gallery.pdel')) {
            $multiactions[] = [$apx->lang->get('CORE_DEL'), 'action.php?action=gallery.pdel&gid='.intval($_REQUEST['id']), false];
        }
        if ($apx->user->has_right('gallery.penable')) {
            $multiactions[] = [$apx->lang->get('CORE_ENABLE'), 'action.php?action=gallery.penable&gid='.intval($_REQUEST['id']), false];
        }
        if ($apx->user->has_right('gallery.pdisable')) {
            $multiactions[] = [$apx->lang->get('CORE_DISABLE'), 'action.php?action=gallery.pdisable&gid='.intval($_REQUEST['id']), false];
        }
    ...
    public function padd()
    {
        ...
        if (1 == $_POST['send']) {
            if (!checkToken()) {
                ...
                //ZIP
                if ('zip' == $_REQUEST['what'] && $_FILES['zip']['tmp_name']) {
            ...
            //Layer
            $layerdef[] = ['LAYER_UPLOAD', 'action.php?action=gallery.padd&amp;id='.$_REQUEST['id'].'&amp;updateparent='.$_REQUEST['updateparent'], !$_REQUEST['what']];
            $layerdef[] = ['LAYER_ZIP', 'action.php?action=gallery.padd&amp;id='.$_REQUEST['id'].'&amp;what=zip&amp;updateparent='.$_REQUEST['updateparent'], 'zip' == $_REQUEST['what']];
            $layerdef[] = ['LAYER_FTP', 'action.php?action=gallery.padd&amp;id='.$_REQUEST['id'].'&amp;what=ftp&amp;updateparent='.$_REQUEST['updateparent'], 'ftp' == $_REQUEST['what']];
    ...
    public function process_files($files)
    {
        ...
        foreach ($files as $file) {
            ...
            //Skalieren
            if (false !== $picture && !$file['noresize'] && $set['gallery']['picwidth'] && $set['gallery']['picheight']) {
                $scaled = $img->resize(
                    $picture,
            ...
            if (false !== $picture && $set['gallery']['watermark'] && $file['watermark']) {
            ...
            if ('ftp' != $_POST['what'] || $_POST['delpics']) {
