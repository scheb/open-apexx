...
class action extends calendar_functions
{
    ...
    public function show()
    {
        ...
        if (($_REQUEST['item'] && ($_REQUEST['title'] || $_REQUEST['text'])) || $_REQUEST['secid'] || $_REQUEST['catid'] || $_REQUEST['userid'] || ($_REQUEST['start_day'] && $_REQUEST['start_month'] && $_REQUEST['start_year']) || ($_REQUEST['end_day'] && $_REQUEST['end_month'] && $_REQUEST['end_year'])) {
            ...
            $ids = get_ids($data, 'id');
            $ids[] = -1;
            $searchid = saveSearchResult('admin_calendar', $ids, [
                'item' => $_REQUEST['item'],
        ...
        $layerdef[] = ['LAYER_RECENT', 'action.php?action=calendar.show', !$_REQUEST['what']];
        $layerdef[] = ['LAYER_SEND', 'action.php?action=calendar.show&amp;what=send', 'send' == $_REQUEST['what']];
        $layerdef[] = ['LAYER_ARCHIVE', 'action.php?action=calendar.show&amp;what=archive', 'archive' == $_REQUEST['what']];
        ...
        $orderdef['cat'] = ['catname', 'ASC', 'COL_CATEGORY'];
        $orderdef['addtime'] = ['a.addtime', 'DESC', 'SORT_ADDTIME'];
        $orderdef['startday'] = ['a.startday', 'ASC', 'SORT_STARTDAY'];
        $orderdef['endday'] = ['a.endday', 'ASC', 'SORT_ENDDAY'];
    ...
    public function show_print($data)
    {
        ...
        $col[] = ['', 1, 'align="center"'];
        $col[] = [$apx->lang->get('COL_TITLE').' / '.$apx->lang->get('COL_USER'), 55, 'class="title"'];
        $col[] = ['COL_CATEGORY', 20, 'align="center"'];
        $col[] = ['COL_STARTEND', 15, 'align="center"'];
    ...
    public function enable()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            list($title) = $db->first('SELECT title FROM '.PRE."_calendar_events WHERE id='".$_REQUEST['id']."' LIMIT 1");
            $apx->tmpl->assign('MESSAGE', $apx->lang->get('MSG_TEXT', ['TITLE' => compatible_hsc($title)]));
            tmessageOverlay('enable', ['ID' => $_REQUEST['id']]);
        }
    }
    ...
    public function catshow()
    {
        ...
        $col[] = ['COL_EVENTS', 25, 'align="center"'];
        ...
        if (count($data)) {
            ...
            foreach ($data as $res) {
                ...
                //Optionen
                if ($apx->user->has_right('calendar.catedit')) {
                    $tabledata[$i]['OPTIONS'] .= optionHTML('edit.gif', 'calendar.catedit', 'id='.$res['id'], $apx->lang->get('CORE_EDIT'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($apx->user->has_right('calendar.catdel') && !$events) {
                    $tabledata[$i]['OPTIONS'] .= '<span class="ifhasnochildren">'.optionHTMLOverlay('del.gif', 'calendar.catdel', 'id='.$res['id'], $apx->lang->get('CORE_DEL')).'</span><span class="ifhaschildren"><img alt="" src="design/ispace.gif"/></span>';
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }

                if ($apx->user->has_right('calendar.catclean') && $events) {
                    $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('clean.gif', 'calendar.catclean', 'id='.$res['id'], $apx->lang->get('CLEAN'));
                } else {
                    $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
                }
    ...
    public function catedit()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            $res = $this->cat->getNode($_REQUEST['id'], ['title', 'icon']);
    ...
    public function catdel()
    {
        ...
        list($events) = $db->first('SELECT count(id) FROM '.PRE."_calendar_events WHERE catid='".$_REQUEST['id']."'");
        if ($events) {
            die('category still contains calendar!');
        }
