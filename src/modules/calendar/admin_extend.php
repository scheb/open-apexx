...
class calendar_functions
{
    ...
    //Daystamp erzeugen
    public function generate_stamp($day, $month, $year)
    {
        return sprintf('%04d%02d%02d', $year, $month, $day);
    }

    //Aus einem Daystamp einen Timestamp machen
    public function stamp2time($stamp)
    {
    ...
    public function explode_stamp($stamp)
    {
    ...
    public function process_pic($tempfile, $filename)
    {
        ...
        if ($width && $height) {
            $pic = $this->img->resize($source, $width, $height, $set['calendar']['pic_quality'], 0);
        } else {
            $pic = $source;
        }
