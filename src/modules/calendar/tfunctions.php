...
//Ähnliche Termine
function calendar_events_similar($tagids = [], $count = 5, $start = 0, $catid = false, $mode = false, $template = 'similar')
{
    global $set,$db,$apx,$user;
    $count = (int) $count;
    $start = (int) $start;
...
function calendar_mini($month = 0, $year = 0, $template = 'minicalendar')
{
    ...
    $year = (int) $year;
    if (!$month) {
        $month = intval(date('n', time() - TIMEDIFF));
    }
    if (!$year) {
        $year = intval(date('Y', time() - TIMEDIFF));
    }
    if ($year > $maxyear) {
        $year = $maxyear;
    }
...
function calendar_stats($template = 'stats')
{
    ...
    if (in_template(['COUNT_EVENTS', 'AVG_HITS'], $parse)) {
