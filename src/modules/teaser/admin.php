...
class action
{
    ...
    public function show()
    {
        ...
        //DnD-Hinweis
        if (1 == $set['teaser']['orderby'] && $apx->user->has_right('articles.edit')) {
        ...
        if (1 != $set['teaser']['orderby']) {
            ...
            $orderdef['hits'] = ['hits', 'ASC', 'COL_HITS'];
        }

        $col[] = ['&nbsp;', 1, ''];
        $col[] = ['COL_TITLE', 50, 'class="title"'];
        $col[] = ['COL_IMAGE', 40, 'align="center"'];
    ...
    public function add()
    {
        ...
        if (1 == $_POST['send']) {
            ...
            if (!checkToken()) {
                infoInvalidToken();
            } elseif (!$_POST['title'] /*|| !$_POST['text']*/ || !$_POST['link']) {
                infoNotComplete();
            } elseif ($_FILES['image']['tmp_name'] && !in_array($ext, ['GIF', 'JPG', 'JPE', 'JPEG', 'PNG'])) {
    ...
    public function group()
    {
        ...
        $col[] = ['COL_TEASERS', 20, 'align="center"'];
        ...
        foreach ($data as $id => $res) {
            ...
            if (!$count) {
                $tabledata[$i]['OPTIONS'] .= optionHTMLOverlay('del.gif', 'teaser.group', 'do=del&id='.$id, $apx->lang->get('CORE_DEL'));
            } else {
                $tabledata[$i]['OPTIONS'] .= '<img src="design/ispace.gif" alt="" />';
            }
        }
