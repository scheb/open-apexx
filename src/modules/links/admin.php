...
function show() {
	...
	if ( ( $_REQUEST['item'] && ( $_REQUEST['title'] || $_REQUEST['url'] || $_REQUEST['text'] ) ) || $_REQUEST['secid'] || $_REQUEST['catid'] || $_REQUEST['userid'] ) {
		...
		if ( $_REQUEST['item'] ) {
			...
			if ( $_REQUEST['url'] ) $sc[]="url LIKE '%".addslashes_like($_REQUEST['item'])."%'";
		...
		$ids = get_ids($data, 'id');
		$ids[] = -1;
		$searchid = saveSearchResult('admin_links', $ids, array(
			'item' => $_REQUEST['item'],
	...
	$layerdef[]=array('LAYER_ALL','action.php?action=links.show',!$_REQUEST['what']);
	$layerdef[]=array('LAYER_SEND','action.php?action=links.show&amp;what=send',$_REQUEST['what']=='send');
	$layerdef[]=array('LAYER_BROKEN','action.php?action=links.show&amp;what=broken',$_REQUEST['what']=='broken');
...
function show_print($data) {
	...
	$col[]=array($apx->lang->get('COL_TITLE').' / '.$apx->lang->get('COL_AUTHOR'),45,'class="title"');
	...
	if ( count($data) ) {
		foreach ( $data AS $res ) {
			...
			//Optionen
			if ( $apx->user->has_right('links.edit') && ( $res['userid']==$apx->user->info['userid'] || $apx->user->has_spright('links.edit') ) ) $tabledata[$i]['OPTIONS'].=optionHTML('edit.gif', 'links.edit', 'id='.$res['id'], $apx->lang->get('CORE_EDIT'));
			else $tabledata[$i]['OPTIONS'].='<img src="design/ispace.gif" alt="" />';
			
			if ( $apx->user->has_right('links.del') && ( $res['userid']==$apx->user->info['userid'] || $apx->user->has_spright('links.del') ) ) $tabledata[$i]['OPTIONS'].=optionHTMLOverlay('del.gif', 'links.del', 'id='.$res['id'], $apx->lang->get('CORE_DEL'));
			else $tabledata[$i]['OPTIONS'].='<img src="design/ispace.gif" alt="" />';
			
			if ( ( !$res['starttime'] || $res['endtime']<time() ) && $apx->user->has_right('links.enable') && ( $res['userid']==$apx->user->info['userid'] || $apx->user->has_spright('links.enable') ) ) $tabledata[$i]['OPTIONS'].=optionHTMLOverlay('enable.gif', 'links.enable', 'id='.$res['id'], $apx->lang->get('CORE_ENABLE'));
			elseif ( $res['starttime'] && $apx->user->has_right('links.disable') && ( $res['userid']==$apx->user->info['userid'] || $apx->user->has_spright('links.disable') ) ) $tabledata[$i]['OPTIONS'].=optionHTMLOverlay('disable.gif', 'links.disable', 'id='.$res['id'], $apx->lang->get('CORE_DISABLE'));
			else $tabledata[$i]['OPTIONS'].='<img src="design/ispace.gif" alt="" />';
			...
			if ( $apx->is_module('comments') ) {
				...
				if ( $comments && ( $apx->is_module('comments') && $set['links']['coms'] ) && $res['allowcoms'] && $apx->user->has_right('comments.show') ) $tabledata[$i]['OPTIONS'].=optionHTML('comments.gif', 'comments.show', 'module=links&mid='.$res['id'], $apx->lang->get('COMMENTS').' ('.$comments.')');
				else $tabledata[$i]['OPTIONS'].='<img src="design/ispace.gif" alt="" />';
			}
			if ( $apx->is_module('ratings') ) {
				list($ratings)=$db->first("SELECT count(id) FROM ".PRE."_ratings WHERE ( module='links' AND mid='".$res['id']."' )");
				if ( $ratings && ( $apx->is_module('ratings') && $set['links']['ratings'] ) && $res['allowrating'] && $apx->user->has_right('ratings.show') ) $tabledata[$i]['OPTIONS'].=optionHTML('ratings.gif', 'ratings.show', 'module=links&mid='.$res['id'], $apx->lang->get('RATINGS').' ('.$ratings.')');
				else $tabledata[$i]['OPTIONS'].='<img src="design/ispace.gif" alt="" />';
			}
	...
	$multiactions = array();
	if ( $apx->user->has_right('links.del') ) $multiactions[] = array($apx->lang->get('CORE_DEL'), 'action.php?action=links.del', false);
	if ( $apx->user->has_right('links.enable') ) $multiactions[] = array($apx->lang->get('CORE_ENABLE'), 'action.php?action=links.enable', false);
	if ( $apx->user->has_right('links.disable') ) $multiactions[] = array($apx->lang->get('CORE_DISABLE'), 'action.php?action=links.disable', false);
...
function add() {
	...
	if ( $_POST['send']==1 ) {
		...
		elseif ( !$_POST['title'] || !$_POST['url'] || !$_POST['catid'] || !$_POST['text'] ) infoNotComplete();
		elseif ( !$this->update_linkpic() ) { /*DO NOTHING*/ }
			...
			//Autor
			if ( !$apx->user->has_spright('links.edit') ) $_POST['userid']=$apx->user->info['userid'];
			...
			if ( $_POST['catid']=='newcat' && $apx->user->has_right('links.catadd') ) return printJSRedirect('action.php?action=links.catadd&addid='.$nid);
			else printJSRedirect('action.php?action=links.show');
		}
	}
	else {
		$_POST['searchable']=1;
...
function edit() {
	...
	if ( $_POST['send']==1 ) {
		...
		elseif ( !$this->update_linkpic() ) { /*DO NOTHING*/ }
			...
			if ( $_POST['catid']=='newcat' && $apx->user->has_right('links.catadd') ) return printJSRedirect('action.php?action=links.catadd&addid='.$_REQUEST['id']);
			else printJSRedirect(get_index('links.show'));
		}
	}
	else {
	...
	//Einzeln
		...
		if ( $_POST['send']==1 ) {
			if ( !checkToken() ) printInvalidToken();
				...
				if ( $apx->is_module('comments') ) $db->query("DELETE FROM ".PRE."_comments WHERE ( module='links' AND mid='".$_REQUEST['id']."' )");
				if ( $apx->is_module('ratings') ) $db->query("DELETE FROM ".PRE."_ratings WHERE ( module='links' AND mid='".$_REQUEST['id']."' )");
	...
	if ( is_array($_REQUEST['multiid']) ) {
		if ( !checkToken() ) printInvalidToken();
			...
			if ( count($cache) ) {
				...
				foreach ( $cache AS $id ) logit('LINKS_ENABLE','ID #'.$id);
			}
	...
	if ( is_array($_REQUEST['multiid']) ) {
		if ( !checkToken() ) printInvalidToken();
			...
			if ( count($cache) ) {
				...
				foreach ( $cache AS $id ) logit('LINKS_DISABLE','ID #'.$id);
			}
...
function catshow() {
	...
	$col[]=array('COL_LINKS',25,'align="center"');
	...
	if ( count($data) ) {
		...
		foreach ( $data AS $res ) {
			...
			//Optionen
			if ( $apx->user->has_right('links.catedit') ) $tabledata[$i]['OPTIONS'].=optionHTML('edit.gif', 'links.catedit', 'id='.$res['id'], $apx->lang->get('CORE_EDIT'));
			else $tabledata[$i]['OPTIONS'].='<img src="design/ispace.gif" alt="" />';
			
			if ( $apx->user->has_right('links.catdel') && !$links ) $tabledata[$i]['OPTIONS'].='<span class="ifhasnochildren">'.optionHTMLOverlay('del.gif', 'links.catdel', 'id='.$res['id'], $apx->lang->get('CORE_DEL')).'</span><span class="ifhaschildren"><img alt="" src="design/ispace.gif"/></span>';
			else $tabledata[$i]['OPTIONS'].='<img src="design/ispace.gif" alt="" />';
			
			if ( $apx->user->has_right('links.catclean') && $links ) $tabledata[$i]['OPTIONS'].=optionHTMLOverlay('clean.gif', 'links.catclean', 'id='.$res['id'], $apx->lang->get('CLEAN'));
			else $tabledata[$i]['OPTIONS'].='<img src="design/ispace.gif" alt="" />';
...
function catedit() {
	...
	if ( $_POST['send']==1 ) {
		...
		elseif ( !$_POST['open'] && $links ) info($apx->lang->get('INFO_CONTAINSLINKS'));
		else {
...
function catdel() {
	...
	list($links)=$db->first("SELECT count(id) FROM ".PRE."_links WHERE catid='".$_REQUEST['id']."'");
	if ( $links ) die('category still contains links!');
