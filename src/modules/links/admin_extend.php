...
class links_functions
{
    ...
    public function update_linkpic()
    {
        global $set,$db,$apx;
    ...
    public function upload_pic()
    {
        ...
        if (!is_uploaded_file($_FILES['pic_upload']['tmp_name'])) {
            die('linkpic has not been uploaded!');
        }
    ...
    public function process_pic($tempfile, $filename)
    {
        ...
        if ($width && $height) {
            $pic = $this->img->resize($source, $width, $height, $set['links']['linkpic_quality'], 0);
        } else {
            $pic = $source;
        }
