...
function glossar_alphabetical($count=5,$start=0,$catid=false,$letter=false,$template='alphabetical') {
	...
	if ( $letter ) {
		...
		if ( $letter=='#' ) $letterfilter=" AND title NOT REGEXP(\"^[a-zA-Z]\")";
		elseif ( $letter=='A' ) $letterfilter="( title LIKE 'A%' OR title LIKE 'Ä%' )";
		elseif ( $letter=='O' ) $letterfilter="( title LIKE 'O%' OR title LIKE 'Ö%' )";
		elseif ( $letter=='U' ) $letterfilter="( title LIKE 'U%' OR title LIKE 'Ü%' )";
		else $letterfilter="title LIKE '".$letter."'%";
	}
...
function glossar_stats($template='stats') {
	...
	if ( in_template(array('COUNT_GLOSSAR', 'AVG_HITS'), $parse) ) {
