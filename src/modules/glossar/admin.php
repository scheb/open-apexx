...
class action
{
    ...
    public function show()
    {
        ...
        if ($_REQUEST['item'] && ($_REQUEST['title'] || $_REQUEST['text'])) {
            ...
            $ids = get_ids($data, 'id');
            $ids[] = -1;
            $searchid = saveSearchResult('admin_glossar', $ids, [
        ...
        //Layer
        $layerdef[] = ['ALL', 'action.php?action=glossar.show', !$_REQUEST['what']];
        $data = $db->fetch('SELECT * FROM '.PRE.'_glossar_cat ORDER BY title ASC');
        if (count($data)) {
            foreach ($data as $res) {
                $layerdef[] = [compatible_hsc($res['title']), 'action.php?action=glossar.show&amp;what='.$res['id'], $_REQUEST['what'] == $res['id']];
            }
        }
    ...
    public function del()
    {
        ...
        if (1 == $_POST['send']) {
            if (!checkToken()) {
                ...
                if ($apx->is_module('comments')) {
                    $db->query('DELETE FROM '.PRE."_comments WHERE ( module='glossar' AND mid='".$_REQUEST['id']."' )");
                }
                if ($apx->is_module('ratings')) {
                    $db->query('DELETE FROM '.PRE."_ratings WHERE ( module='glossar' AND mid='".$_REQUEST['id']."' )");
                }
    ...
    public function catshow()
    {
        ...
        $col[] = ['COL_CATNAME', 72, 'class="title"'];
        $col[] = ['COL_ENTRIES', 25, 'align="center"'];
    ...
    public function catdel()
    {
        ...
        if ($entries) {
            die('category still contains entries!');
        }
