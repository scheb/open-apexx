...
function glossar_highlight($text, $module = false, $ignore = false)
{
    ...
    if (!isset($highlights)) {
        ...
        if (count($data)) {
            foreach ($data as $res) {
                ...
                $content = strtr(compatible_hsc($content), [
                    "\n" => ' ',
                    "\r" => '',
                    '\'' => '\\\'',
                    '\\' => '\\\\',
    ...
    foreach ($highlights as $element) {
        ...
        foreach ($words as $wkey => $word) {
            if (false === strpos($lowertext, strtolower($word))) {
                unset($words[$wkey]);
            }
        }
        if (!count($words)) {
        ...
        $text = preg_replace('#((<[^>]*)|($|[\s<>,.:;_!-])('.$searchfor.')([\s<>,.:;_!-]|$))#ie', '"\2"=="\1" ? glossar_stripslashes("\1") : glossar_stripslashes("\3")."<span class=\"'.$classname_word.'\" onmouseover=\"'.strtr($hover, ['\\' => '\\\\']).'\">".glossar_stripslashes("\4")."</span>".glossar_stripslashes("\5")', $text);
