...
//Artikel-Typ prüfen
if ( !$arttype ) $arttype='normal';
if ( !in_array($arttype,array('normal','preview','review')) ) die('invalid article type!');

//MySQL-Filter erstellen
if ( $arttype!='normal' || $set['articles']['normalonly'] ) $artfilter=" AND type='".$arttype."' ";
else $artfilter='';

//Wohin verlinken?
if ( $arttype=='normal' ) $link2file='articles';
else $link2file=$arttype.'s';
...
if ( $_REQUEST['id'] && $_REQUEST['pic'] ) {
	...
	if ( !is_array($article['pictures']) ) require('lib/_end.php'); //Kein Bilder vorhanden
	...
	if ( !$pic ) filenotfound(); //Bild nicht gefunden
...
if ( $_REQUEST['id'] ) {
	...
	if ( $set['articles']['reviews_conclusionpage'] && $arttype=='review' ) $page_count+=1;
	if ( $set['articles']['previews_conclusionpage'] && $arttype=='preview' ) $page_count+=1;
	
	//INFO: Fazitseite
	if ( ( ( $set['articles']['reviews_conclusionpage'] && $arttype=='review' ) || ( $set['articles']['previews_conclusionpage'] && $arttype=='preview' ) ) && $_REQUEST['page']==$page_count ) {
		$conclusionpage=true;
	}
	...
	//INFO: Aktuelle Seite
		...
		if ( !$page['id'] ) filenotfound();
	}
	
	//Verwendete Variablen auslesen
	if ( $conclusionpage ) $template='conclusion_'.$arttype.'s';
	else $template='page'.iif($arttype!='normal','_'.$arttype.'s');
	...
	//Normale Seite
		...
		if ( $apx->is_module('glossar') ) $pagetext = glossar_highlight($pagetext);
	...
	if ( $arttype=='preview' ) {
		...
		if ( in_array('CONCLUSION',$parse) ) {
			...
			if ( $apx->is_module('glossar') ) $conclusion = glossar_highlight($conclusion);
			$apx->tmpl->assign('CONCLUSION',$conclusion);
		}
	}
	
	//Besondere Platzhalter für REVIEWS
	elseif ( $arttype=='review' ) {
		...
		if ( in_array('CONCLUSION',$parse) ) $apx->tmpl->assign('CONCLUSION',mediamanager_inline($review['conclusion']));
	...
	if ( $apx->is_module('comments') && $set['articles']['coms'] && $article['allowcoms'] ) {
		...
		if ( !articles_is_recent($article['id']) && !$set['articles']['archcoms'] ) $apx->tmpl->assign('COMMENT_NOFORM',1);
	}
	...
	if ( $apx->is_module('ratings') && $set['articles']['ratings'] && $article['allowrating'] ) {
		...
		if ( !articles_is_recent($article['id']) && !$set['articles']['archratings'] ) $apx->tmpl->assign('RATING_NOFORM',1);
	}
...
if ( count($data) ) {
	foreach ( $data AS $res ) {
		...
		if ( in_array('ARTICLE.TEXT',$parse) ) {
			...
			if ( $apx->is_module('glossar') ) $page1text = glossar_highlight($page1text);
		}
		...
		if ( $apx->is_module('comments') && $set['articles']['coms'] && $res['allowcoms'] ) {
			require_once(BASEDIR.getmodulepath('comments').'class.comments.php');
			if ( !isset($coms) ) $coms=new comments('articles',$res['id']);
		...
		if ( $apx->is_module('ratings') && $set['articles']['ratings'] && $res['allowrating'] ) {
			require_once(BASEDIR.getmodulepath('ratings').'class.ratings.php');
			if ( !isset($rate) ) $rate=new ratings('articles',$res['id']);
...
$apx->tmpl->parse('index'.iif($arttype!='normal','_'.$arttype.'s'));
