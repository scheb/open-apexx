...
if (!isset($_POST['searchin']) || (!is_array($_POST['searchin']) && 'all' == $_POST['searchin'])) {
    $_POST['searchin'] = [];
...
if ($_POST['send']) {
    ...
    if (count($items)) {
        ...
        foreach ($_POST['searchin'] as $module => $value) {
            ...
            if (isset($set[$module]['searchable']) && !$set[$module]['searchable']) {
                continue;
            }
            ++$i;
...
foreach ($apx->modules as $module => $info) {
    ...
    if (!function_exists('search_'.$module)) {
        continue;
    }
    if (isset($set[$module]['searchable']) && !$set[$module]['searchable']) {
        continue;
    ...
    $posdata[$i]['CHECKED'] = iif('1' == $_POST['searchin'][$module], 1, 0);
}
...
$apx->tmpl->assign('CONN', iif('and' == $_POST['conn'] || 'or' == $_POST['conn'], $_POST['conn'], 'and'));
