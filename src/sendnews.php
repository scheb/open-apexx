...
if ( $_POST['send'] ) {
	...
	elseif ( ( !$_POST['send_username'] && !$user->info['userid'] ) || !$_POST['catid'] || !$_POST['title'] || !$_POST['text'] || ( ( $_POST['source_title'] || $_POST['source_url'] ) && ( !$_POST['source_title'] || !$_POST['source_url'] ) ) ) message('back');
	elseif ( ($spam+$set['news']['spamprot']*60)>time() ) message($apx->lang->get('MSG_BLOCKSPAM',array('SEC'=>($spam+$set['news']['spamprot']*60)-time())),'back');
	else {
