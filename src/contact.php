...
if ($_POST['send']) {
    ...
    foreach ($_POST as $key => $value) {
        if (in_array($key, ['name', 'email', 'copytome', 'sendto', 'subject', 'text', 'send', 'capcha', 'captcha', 'capcha_hash', 'captcha_hash'])) {
            continue;
        }
    ...
    } elseif (!$_POST['name'] || !$_POST['email'] || !$_POST['sendto'] || !$_POST['subject'] || !$_POST['text'] || $addnl_failed) {
        message('back');
...
//Formular anzeigen
if ($_REQUEST['form'] && preg_match('#^[a-zA-Z0-9_-]+$#', $_REQUEST['form']) && file_exists(BASEDIR.getpath('tmpl_modules_public', ['MODULE' => 'contact', 'THEME' => $apx->tmpl->theme]).$_REQUEST['form'].'.html')) {
