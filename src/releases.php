...
if (count($data)) {
    ...
    $groupinfo = [];
    $groups = [];
    if (in_template(['PRODUCT.MEDIA', 'PRODUCT.MEDIA_ICON'], $parse)) {
        $groups = array_merge($groups, get_ids($data, 'media'));
    }
    if (in_array('PRODUCT.GENRE', $parse)) {
        $groups = array_merge($groups, get_ids($data, 'genre'));
    }
