...
if ($_REQUEST['pic']) {
    ...
    if (!$_REQUEST['comments']) {
        $db->query('UPDATE '.PRE."_gallery_pics SET hits=hits+1 WHERE id='".$_REQUEST['pic']."' LIMIT 1");
    }
...
HTML;
    ...
    if ($password && $password == $_POST['password']) {
        ...
        tmessage('pwdrequired', ['ID' => $_REQUEST['id'], 'PIC' => $_REQUEST['pic']]);
    }
...
if ($_REQUEST['id']) {
    ...
    if ($set['gallery']['subgals'] && $gallery['children']) {
        ...
        $openIds = array_merge(get_ids($openData), [$gallery['id']]);
    } else {
        $openIds = [$gallery['id']];
    }
    ...
    if ($set['gallery']['subgals']) {
        ...
        $data = $tree->getLevel(['*'], $_REQUEST['id'], "'".time()."' BETWEEN starttime AND endtime");
        if (count($data)) {
            foreach ($data as $res) {
                ++$i;
                $subtreeIds = array_merge($res['children'], [$res['id']]);
                ...
                if (in_template(['GALLERY.COUNT', 'GALLERY.UPDATETIME'], $parse)) {
                ...
                //Vorschau-Bild
                if (!$res['password'] && in_template(['GALLERY.PREVIEW', 'GALLERY.PREVIEW_FULLSIZE'], $parse)) {
    ...
    if (in_template(['COUNT', 'UPDATETIME'], $parse)) {
    ...
    if (count($data)) {
        foreach ($data as $res) {
            ...
            //Neue Bilder
            if (($res['addtime'] + ($set['gallery']['new'] * 24 * 3600)) >= time()) {
                $new = 1;
            } else {
                $new = 0;
            }
            ...
            if ($apx->is_module('comments') && $set['gallery']['coms'] && $res['allowcoms']) {
                ...
                if (in_template(['PICTURE.COMMENT_LAST_USERID', 'PICTURE.COMMENT_LAST_NAME', 'PICTURE.COMMENT_LAST_TIME'], $parse)) {
...
if ($set['gallery']['subgals']) {
    require_once BASEDIR.'lib/class.recursivetree.php';
    $tree = new RecursiveTree(PRE.'_gallery', 'id');
    $data = $tree->getLevel(['*'], 0, "'".time()."' BETWEEN starttime AND endtime ".section_filter(true, 'secid'));
} else {
    if ($_REQUEST['letter']) {
        $orderby = 'title ASC';
    } elseif (2 == $set['gallery']['ordergal']) {
        $orderby = 'title ASC';
...
if (count($data)) {
    foreach ($data as $res) {
        ...
        if ($set['gallery']['subgals'] && $set['gallery']['listepp']) {
            if (!($i > ($_REQUEST['p'] - 1) * $set['gallery']['listepp'] && $i <= $_REQUEST['p'] * $set['gallery']['listepp'])) {
                continue;
            }
        }
