...
if ( $_REQUEST['id'] ) {
	...
	$apx->tmpl->assign('SOURCE',$res['source']=='external' ? 'apexx' : $res['source']);
	...
	//Video-Limit
	if ( videos_limit_is_reached($res['id'],$res['limit']) ) $apx->tmpl->assign('LIMIT',1);
...
if ( $_REQUEST['action']=='search' ) {
	...
	if ( $_REQUEST['searchid'] ) {
		...
		//Sortierung
		if ( $set['videos']['sortby']==2 ) $orderby=' ORDER BY starttime DESC ';
...
//Filter bestimmen
if ( $set['videos']['catonly'] ) $filter="catid='".$_REQUEST['catid']."'";
...
//Orderby
if ( $set['videos']['sortby']==2 ) $orderdef[0]='date';
