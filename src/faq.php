...
if ($_REQUEST['id']) {
    ...
    $node = $tree->getNode($_REQUEST['id'], ['*']);
    if (!$node || (!$user->is_team_member() && 0 == $node['starttime'])) {
        filenotfound();
    }
    $node['level'] = 0;
    $subData = $tree->getTree(['*'], $_REQUEST['id'], "starttime!='0'");
    $data = array_merge([$node], $subData);
} else {
    $data = $tree->getTree(['*'], null, "starttime!='0'");
}
