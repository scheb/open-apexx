...
if ( $_REQUEST['action']=='search' ) {
	...
	if ( $_REQUEST['searchid'] ) {
		...
		foreach ( $data AS $res ) {
			...
			if ( in_array('ARTICLE.TEASER',$parse) ) $tabledata[$i]['TEASER']=mediamanager_inline($res['teaser']);
			if ( in_array('ARTICLE.TEXT',$parse) ) $tabledata[$i]['TEXT']=mediamanager_inline($page1text);
	...
	//SUCHE DURCHFÜHREN
		...
		foreach ( $_REQUEST['type'] AS $type ) {
			if ( in_array($type,array('normal','preview','review')) ) $arttypes[]="'".$type."'";
		}
...
if ( $_REQUEST['month'] ) {
	...
	//Artikel ausgeben
	if ( $set['articles']['archiveentrysort']==1 ) $orderby=' starttime DESC ';
