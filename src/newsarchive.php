    ...
    if ($_REQUEST['searchid']) {
        ...
        foreach ($data as $res) {
            ...
            if (in_array('NEWS.TEXT', $parse)) {
                $tabledata[$i]['TEXT'] = mediamanager_inline($res['text']);
            }
            ...
            if ($apx->is_module('comments') && $set['news']['coms'] && $res['allowcoms']) {
                ...
                if (in_template(['NEWS.COMMENT_LAST_USERID', 'NEWS.COMMENT_LAST_NAME', 'NEWS.COMMENT_LAST_TIME'], $parse)) {
...
if ($_REQUEST['month']) {
    ...
    //News ausgeben
    if (1 == $set['news']['archiveentrysort']) {
...
$data = $db->fetch('SELECT starttime FROM '.PRE."_news WHERE ( '".time()."' BETWEEN starttime AND endtime ".$filter.' '.section_filter().' ) ORDER BY starttime '.iif(2 == $set['news']['archivesort'], 'ASC', 'DESC'));
if (count($data)) {
    foreach ($data as $res) {
