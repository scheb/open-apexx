...
if (!$_REQUEST['year']) {
    if ('day' == $set['calendar']['start']) {
        $_REQUEST['day'] = date('d', time() - TIMEDIFF);
        $_REQUEST['month'] = date('m', time() - TIMEDIFF);
        $_REQUEST['year'] = date('Y', time() - TIMEDIFF);
    } elseif ('week' == $set['calendar']['start']) {
        $_REQUEST['week'] = date('W', time() - TIMEDIFF);
        $_REQUEST['year'] = date('Y', time() - TIMEDIFF);
    } elseif ('month' == $set['calendar']['start']) {
        $_REQUEST['month'] = date('m', time() - TIMEDIFF);
        $_REQUEST['year'] = date('Y', time() - TIMEDIFF);
    }
}
...
//Modus
if (!in_array($_REQUEST['mode'], ['public', 'private']) || !$user->info['userid']) {
    $_REQUEST['mode'] = 'public';
}
if ('private' == $_REQUEST['mode']) {
    $modefilter = " AND private='1' AND userid='".$user->info['userid']."' ";
} else {
    $modefilter = " AND private='0' ";
}

//Headline
$callink = mklink('calendar.php', 'calendar.html');
if ('private' == $_REQUEST['mode']) {
    $callink = mklink('calendar.php?mode=private', 'calendar,private.html');
}
...
function calendar_events_from_day($thisdaystamp, $parse, $parseprefix = 'DAY.')
{
    ...
    if (!isset($catdata)) {
        $catdata = [];
        if (in_template([$parseprefix.'EVENT.CATTITLE', $parseprefix.'EVENT.CATICON'], $parse)) {
    ...
    if (count($edata)) {
        foreach ($edata as $res) {
            ...
            if (in_template([$parseprefix.'EVENT.PICTURE', $parseprefix.'EVENT.PICTURE_POPUP', $parseprefix.'EVENT.PICTURE_POPUPPATH'], $parse)) {
                list($picture, $picture_popup, $picture_popuppath) = calendar_pic($res['picture']);
            }

            //Start berechnen
            $startday = $starttime = $endday = $endtime = 0;
            if (in_template([$parseprefix.'EVENT.STARTDAY', $parseprefix.'EVENT.STARTTIME'], $parse)) {
            ...
            if (in_template([$parseprefix.'EVENT.ENDDAY', $parseprefix.'EVENT.ENDTIME'], $parse)) {
                if ($res['endday'] != $res['startday'] || -1 != $res['endtime']) {
...
function calendar_birthdays_from_day($timestamp, $parse)
{
    ...
    if (count($bdata)) {
        foreach ($bdata as $res) {
            ...
            if ($bd[2]) {
                $birthdata[$i]['AGE'] = date('Y', $timestamp - TIMEDIFF) - $bd[2];
            }
        }
    }
...
if ($_REQUEST['day'] && $_REQUEST['month'] && $_REQUEST['year']) {
    ...
    headline($daytitle, mklink(
        'calendar.php?day='.$day.'&amp;month='.$month.'&amp;year='.$year.iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
    ...
    if ($user->info['userid']) {
        ...
        ).iif('private' == $_REQUEST['mode'], '&amp;private=1');
    }
    ...
    if (!(1 == $day && 1 == $month && $year == $minyear)) {
        $link_previous = mklink(
            'calendar.php?day='.date('j', $prevdaystamp).'&amp;month='.date('n', $prevdaystamp).'&amp;year='.date('Y', $prevdaystamp).iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
            'calendar,'.$_REQUEST['mode'].',day'.date('dmY', $prevdaystamp).'.html'
        );
    }
    if (!(31 == $day && 12 == $month && $year == $maxyear)) {
        $link_next = mklink(
            'calendar.php?day='.date('j', $nextdaystamp).'&amp;month='.date('n', $nextdaystamp).'&amp;year='.date('Y', $nextdaystamp).iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
    ...
    //Ansichtswechsel
    $link_today = mklink('calendar.php?today=1'.iif('private' == $_REQUEST['mode'], '&amp;mode=private'), 'calendar,'.$_REQUEST['mode'].',today.html');
    $link_week = mklink('calendar.php?week='.$weeknumber.'&amp;year='.$year.iif('private' == $_REQUEST['mode'], '&amp;mode=private'), 'calendar,'.$_REQUEST['mode'].',week'.sprintf('%02d', $weeknumber).$year.'.html');
    $link_month = mklink('calendar.php?month='.$month.'&amp;year='.$year.iif('private' == $_REQUEST['mode'], '&amp;mode=private'), 'calendar,'.$_REQUEST['mode'].',month'.sprintf('%02d', $month).$year.'.html');
    $link_year = mklink('calendar.php?year='.$year.iif('private' == $_REQUEST['mode'], '&amp;mode=private'), 'calendar,'.$_REQUEST['mode'].',year'.$year.'.html');
...
if ($_REQUEST['week'] && $_REQUEST['year']) {
    ...
    headline($week.'. '.$apx->lang->get('CALWEEK').' '.$year, mklink(
        'calendar.php?week='.date('W', $time_thisweek - TIMEDIFF).'&amp;year='.date('Y', $time_thisweek).iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
    ...
    for ($i = 0; $i < 7; ++$i) {
        ...
        //Link zum Tag
        $link = mklink(
            'calendar.php?day='.date('j', $timestamp - TIMEDIFF).'&amp;month='.date('n', $timestamp - TIMEDIFF).'&amp;year='.date('Y', $timestamp - TIMEDIFF).iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
    ...
    if (date('Y', $time_prevweek - TIMEDIFF) >= $minyear) {
        $link_previous = mklink(
            'calendar.php?week='.date('W', $time_prevweek - TIMEDIFF).'&amp;year='.date('Y', $time_prevweek).iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
    ...
    if (date('Y', $time_nextweek - TIMEDIFF) <= $maxyear) {
        $link_next = mklink(
            'calendar.php?week='.date('W', $time_nextweek - TIMEDIFF).'&amp;year='.date('Y', $time_nextweek).iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
    ...
    //Ansichtswechsel
    $link_today = mklink('calendar.php?today=1'.iif('private' == $_REQUEST['mode'], '&amp;mode=private'), 'calendar,'.$_REQUEST['mode'].',today.html');
    $link_week = mklink('calendar.php?week='.$week.'&amp;year='.$year.iif('private' == $_REQUEST['mode'], '&amp;mode=private'), 'calendar,'.$_REQUEST['mode'].',week'.sprintf('%02d', $week).$_REQUEST['year'].'.html');
    $link_month = mklink('calendar.php?month='.date('n', $firrstday - TIMEDIFF).'&amp;year='.$year.iif('private' == $_REQUEST['mode'], '&amp;mode=private'), 'calendar,'.$_REQUEST['mode'].',month'.date('m', $firrstday - TIMEDIFF).$year.'.html');
    $link_year = mklink('calendar.php?year='.$year.iif('private' == $_REQUEST['mode'], '&amp;mode=private'), 'calendar,'.$_REQUEST['mode'].',year'.$year.'.html');
    $apx->tmpl->assign('LINK_SHOWTODAY', $link_today);
    $apx->tmpl->assign('LINK_SHOWWEEK', $link_week);
    $apx->tmpl->assign('LINK_SHOWMONTH', $link_month);
...
if ($_REQUEST['year'] && !$_REQUEST['month']) {
    ...
    headline($year, mklink(
        'calendar.php?year='.$year.iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
    ...
    for ($month = 1; $month <= 12; ++$month) {
        ...
        //Link zum Monat
        $link = mklink(
            'calendar.php?month='.date('n', $timestamp - TIMEDIFF).'&amp;year='.date('Y', $timestamp - TIMEDIFF).iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
        ...
        for ($day = 1 - $subday; $day <= $monthdays + $adddays; ++$day) {
            ...
            //Link: Woche anzeigen
            if (!$weeklink || 1 == date('w', $timestamp - TIMEDIFF)) {
                $weektimestamp = $timestamp;
                $weektimestamp -= (date('w', $timestamp - TIMEDIFF) - 4) * 24 * 3600;
                $weeknumber = date('W', $weektimestamp - TIMEDIFF);
                $weekyear = date('Y', $weektimestamp - TIMEDIFF);
                $weeklink = mklink(
                    'calendar.php?week='.date('W', $timestamp - TIMEDIFF).'&amp;year='.date('Y', $timestamp - TIMEDIFF).iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
                    'calendar,'.$_REQUEST['mode'].',week'.sprintf('%02d', $weeknumber).$weekyear.'.html'
                );
                if (!$firstweekofmonth) {
                    $firstweekofmonth = $weeklink;
                }
            }
    ...
    if ($year >= $minyear) {
        $link_previous = mklink(
            'calendar.php?year='.($year - 1).iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
    ...
    //Ansichtswechsel
    $link_today = mklink('calendar.php?today=1'.iif('private' == $_REQUEST['mode'], '&amp;mode=private'), 'calendar,'.$_REQUEST['mode'].',today.html');
    $link_week = mklink('calendar.php?week=1&amp;year='.$year.iif('private' == $_REQUEST['mode'], '&amp;mode=private'), 'calendar,'.$_REQUEST['mode'].',week01'.$year.'.html');
    $link_month = mklink('calendar.php?month=1&amp;year='.$year.iif('private' == $_REQUEST['mode'], '&amp;mode=private'), 'calendar,'.$_REQUEST['mode'].',month01'.$year.'.html');
    $link_year = mklink('calendar.php?year='.$year.iif('private' == $_REQUEST['mode'], '&amp;mode=private'), 'calendar,'.$_REQUEST['mode'].',year'.$year.'.html');
    $apx->tmpl->assign('LINK_SHOWTODAY', $link_today);
    $apx->tmpl->assign('LINK_SHOWWEEK', $link_week);
    $apx->tmpl->assign('LINK_SHOWMONTH', $link_month);
...
headline($monthtitle, mklink(
    'calendar.php?month='.$month.'&amp;year='.$year.iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
...
for ($day = 1 - $subday; $day <= $monthdays + $adddays; ++$day) {
    ...
    if (!$weeklink || 1 == date('w', $timestamp - TIMEDIFF)) {
        ...
        $weekyear = date('Y', $weektimestamp - TIMEDIFF);
        $weeklink = mklink(
            'calendar.php?week='.$weeknumber.'&amp;year='.$weekyear.iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
            'calendar,'.$_REQUEST['mode'].',week'.sprintf('%02d', $weeknumber).$weekyear.'.html'
        );
        if (!$firstweekofmonth) {
            $firstweekofmonth = $weeklink;
...
if (!(1 == $month && $year == $minyear)) {
    ...
    $link_previous = mklink(
        'calendar.php?month='.$prevmonth.'&amp;year='.$prevyear.iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
        'calendar,'.$_REQUEST['mode'].',month'.sprintf('%02d', $prevmonth).$prevyear.'.html'
    );
}
if (!(12 == $month && $year == $maxyear)) {
    if (12 == $month) {
        $nextmonth = 1;
        $nextyear = $year + 1;
    } else {
    ...
    $link_next = mklink(
        'calendar.php?month='.$nextmonth.'&amp;year='.$nextyear.iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
