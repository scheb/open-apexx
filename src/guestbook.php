...
function text_is_blocked()
{
    ...
    if (!is_array($set['guestbook']['blockstring'])) {
        $set['guestbook']['blockstring'] = [];
    }
...
if ($_REQUEST['add']) {
    ...
    if ($_POST['send']) {
        ...
        if ($user->info['userid']) {
            ...
            } elseif (ip_is_blocked()) {
                message($apx->lang->get('MSG_BLOCKIP'), 'back');
            } elseif (!$_POST['text'] || ($set['guestbook']['req_title'] && !$_POST['title'])) {
                message('back');
            } elseif (text_is_blocked()) {
                message($apx->lang->get('MSG_BLOCKTEXT'), 'back');
            } elseif ($set['guestbook']['entrymaxlen'] && strlen($_POST['text']) > $set['guestbook']['entrymaxlen']) {
                message($apx->lang->get('MSG_TOOLONG'), 'back');
            } elseif (($spam + $set['guestbook']['spamprot'] * 60) > time()) {
                message($apx->lang->get('MSG_BLOCKSPAM', ['SEC' => ($spam + $set['guestbook']['spamprot'] * 60) - time()]), 'back');
            } else {
                if ($set['guestbook']['mod'] && !$user->is_team_member()) {
                    $_POST['active'] = 0;
                } else {
                    $_POST['active'] = 1;
                }
            ...
            if ($captchafailed) {
                ...
                message('back');
            } elseif ($set['guestbook']['maxlen'] && strlen($_POST['text']) > $set['guestbook']['maxlen']) {
                message($apx->lang->get('MSG_TOOLONG'), 'back');
            } elseif (text_is_blocked()) {
                message($apx->lang->get('MSG_BLOCKTEXT'), 'back');
            } elseif ($emailnotvalid) {
                message($apx->lang->get('MSG_EMAILNOTVALID'), 'back');
            } elseif (($spam + $set['guestbook']['spamprot'] * 60) > time()) {
                message($apx->lang->get('MSG_BLOCKSPAM', ['SEC' => ($spam + $set['guestbook']['spamprot'] * 60) - time()]), 'back');
            } else {
