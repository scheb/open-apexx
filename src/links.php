...
if ( $_REQUEST['id'] ) {
	...
	//Neu?
	if ( ($res['addtime']+($set['links']['new']*24*3600))>=time() ) $new=1;
	else $new=0;
...
if ( $_REQUEST['action']=='search' ) {
	...
	if ( $_REQUEST['searchid'] ) {
		...
		//Sortierung
		if ( $set['links']['sortby']==2 ) $orderby=' ORDER BY starttime DESC ';
		...
		$catinfo=array();
		if ( count($catids) && in_template(array('LINK.CATTITLE','LINK.CATTEXT','LINK.CATICON'),$parse) ) {
...
//Filter bestimmen
if ( $set['links']['catonly'] ) $filter="catid='".$_REQUEST['catid']."'";
