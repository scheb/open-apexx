...
if ( $_POST['send'] ) {
	...
	elseif ( ( !$_POST['send_username'] && !$user->info['userid'] ) || !$_POST['catid'] || !$_POST['start_day'] || !$_POST['start_month'] || !$_POST['start_year'] ) message('back');
	elseif ( ($spam+1*60)>time() ) message($apx->lang->get('MSG_BLOCKSPAM',array('SEC'=>($spam+1*60)-time())),'back');
	else {
