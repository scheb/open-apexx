...
if ('private' == $_REQUEST['mode']) {
    $eventslink = mklink('events.php?mode=private', 'events,private.html');
}
...
if ($_REQUEST['add']) {
    ...
    //Termin eintragen
    if ($_POST['send']) {
        if (!$_POST['title'] || !$_POST['text'] || !$_POST['start_day'] || !$_POST['start_month'] || !$_POST['start_year']) {
            message('back');
        } else {
...
if ($_REQUEST['del']) {
    ...
    //Formular zeigen
        ...
        tmessage('delevent', ['ID' => $_REQUEST['del']]);
    }
...
if ($_REQUEST['search']) {
    ...
    //Suche durchführen
    if ('POST' == $_SERVER['REQUEST_METHOD'] || $_REQUEST['tag']) {
        if (
    ...
    if ($_REQUEST['searchid']) {
        ...
        //Kategorien auslesen
        $catdata = [];
        if (in_template(['EVENT.CATTITLE', 'EVENT.CATICON'], $parse)) {
        ...
        foreach ($data as $res) {
            ...
            if (in_template(['EVENT.PICTURE', 'EVENT.PICTURE_POPUP', 'EVENT.PICTURE_POPUPPATH'], $parse)) {
                list($picture, $picture_popup, $picture_popuppath) = calendar_pic($res['picture']);
            }

            //Start berechnen
            $startday = $starttime = $endday = $endtime = 0;
            if (in_template(['EVENT.STARTDAY', 'EVENT.STARTTIME'], $parse)) {
            ...
            if (in_template(['EVENT.ENDDAY', 'EVENT.ENDTIME'], $parse)) {
                if ($res['endday'] != $res['startday'] || -1 != $res['endtime']) {
            ...
            //Galerie
            if ($apx->is_module('gallery') && $res['galid'] && !$res['private'] && in_template(['EVENT.GALLERY_ID', 'EVENT.GALLERY_TITLE', 'EVENT.GALLERY_LINK'], $parse)) {
            ...
            if ($apx->is_module('comments') && $set['calendar']['coms'] && $res['allowcoms'] && !$res['private']) {
                ...
                if (in_template(['EVENT.COMMENT_LAST_USERID', 'EVENT.COMMENT_LAST_NAME', 'EVENT.COMMENT_LAST_TIME'], $parse)) {
...
if ($_REQUEST['id']) {
    ...
    //Aufmacher
    $picture = $picture_popup = $picture_popuppath = '';
    if (in_template(['PICTURE', 'PICTURE_POPUP', 'PICTURE_POPUPPATH'], $parse)) {
        list($picture, $picture_popup, $picture_popuppath) = calendar_pic($res['picture']);
    }

    //Start berechnen
    $startday = $starttime = $endday = $endtime = 0;
    if (in_template(['STARTDAY', 'STARTTIME'], $parse)) {
        $startday = calendar_stamp2time($res['startday']);
        if (-1 != $res['starttime']) {
            $time_comp = calendar_explode_stamp($res['startday']);
            $tmpstamp = sprintf('%04d', $res['starttime']);
    ...
    //Ende berechnen (falls nötig)
    if (in_template(['ENDDAY', 'ENDTIME'], $parse)) {
        if ($res['endday'] != $res['startday'] || -1 != $res['endtime']) {
            $endday = calendar_stamp2time($res['endday']);
            if (-1 != $res['endtime']) {
                $time_comp = calendar_explode_stamp($res['endday']);
    ...
    //Teilnahme
    $takepart = false;
    if (in_template(['LINK_PART_ADD', 'LINK_PART_REMOVE'], $parse) && $user->info['userid'] && floatval($res['endday'].sprintf('%04d', iif(-1 != $res['endtime'], $res['endtime'], 9999))) >= floatval(date('YmdHi', time() - TIMEDIFF))) {
        if ($user->info['userid']) {
            list($takepart) = $db->first('SELECT userid FROM '.PRE."_calendar_parts WHERE userid='".$user->info['userid']."' AND eventid='".$res['id']."' LIMIT 1");
        }
        if ($takepart) {
...
for ($cday = 1 - $subday; $cday <= $monthdays + $adddays; ++$cday) {
    ...
    //Tag auswählen
    $link = mklink(
        'events.php?day='.date('j', $timestamp - TIMEDIFF).'&amp;month='.date('n', $timestamp - TIMEDIFF).'&amp;year='.date('Y', $timestamp - TIMEDIFF).iif('private' == $_REQUEST['mode'], '&amp;mode=private'),
...
//Kategorien auslesen
$catdata = [];
if (in_template(['DAY.EVENT.CATTITLE', 'DAY.EVENT.CATICON'], $parse)) {
    $catdata = $db->fetch_index('SELECT * FROM '.PRE.'_calendar_cat', 'id');
}
...
for ($addday = 0; $addday < $set['calendar']['eventdays']; ++$addday) {
    ...
    //Sortby
    if (2 == $set['calendar']['sortby']) {
        $sortby = ' title ASC, priority ASC ';
    } else {
        $sortby = ' startday DESC, starttime ASC, priority ASC ';
    }
    ...
    if (count($data)) {
        foreach ($data as $res) {
            ...
            //Aufmacher
            $picture = $picture_popup = $picture_popuppath = '';
            if (in_template(['DAY.EVENT.PICTURE', 'DAY.EVENT.PICTURE_POPUP', 'DAY.EVENT.PICTURE_POPUPPATH'], $parse)) {
                list($picture, $picture_popup, $picture_popuppath) = calendar_pic($res['picture']);
            }

            //Start berechnen
            $startday = $starttime = $endday = $endtime = 0;
            if (in_template(['DAY.EVENT.STARTDAY', 'DAY.EVENT.STARTTIME'], $parse)) {
                $startday = calendar_stamp2time($res['startday']);
                if (-1 != $res['starttime']) {
                    $time_comp = calendar_explode_stamp($res['startday']);
                    $tmpstamp = sprintf('%04d', $res['starttime']);
            ...
            //Ende berechnen (falls nötig)
            if (in_template(['DAY.EVENT.ENDDAY', 'DAY.EVENT.ENDTIME'], $parse)) {
                if ($res['endday'] != $res['startday'] || -1 != $res['endtime']) {
                    $endday = calendar_stamp2time($res['endday']);
                    if (-1 != $res['endtime']) {
                        $time_comp = calendar_explode_stamp($res['endday']);
            ...
            //Galerie
            if ($apx->is_module('gallery') && $res['galid'] && !$res['private'] && in_template(['DAY.EVENT.GALLERY_ID', 'DAY.EVENT.GALLERY_TITLE', 'DAY.EVENT.GALLERY_LINK'], $parse)) {
                $galinfo = gallery_info($res['galid']);
                $event['GALLERY_ID'] = $galinfo['id'];
                $event['GALLERY_TITLE'] = $galinfo['title'];
                $event['GALLERY_LINK'] = mklink(
            ...
            if ($apx->is_module('comments') && $set['calendar']['coms'] && $res['allowcoms'] && !$res['private']) {
                ...
                $event['COMMENT_LINK'] = $coms->link($link);
                $event['DISPLAY_COMMENTS'] = 1;
                if (in_template(['DAY.EVENT.COMMENT_LAST_USERID', 'DAY.EVENT.COMMENT_LAST_NAME', 'DAY.EVENT.COMMENT_LAST_TIME'], $parse)) {
                    $event['COMMENT_LAST_USERID'] = $coms->last_userid();
                    $event['COMMENT_LAST_NAME'] = $coms->last_name();
                    $event['COMMENT_LAST_TIME'] = $coms->last_time();
                }
...
if ($user->info['userid']) {
    $apx->tmpl->assign('LINK_ADDEVENT', mklink('events.php?add=1', 'events,add.html'));
}
