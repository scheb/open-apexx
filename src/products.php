...
if ('search' == $_REQUEST['action']) {
    ...
    if ($_REQUEST['searchid']) {
        ...
        //Produkte auslesen
        if (1 != $set['products']['sortby'] || 'release.DESC' == $_REQUEST['sortby']) {
            $data = $db->fetch('SELECT a.*,min(stamp) AS minrelease,IF(b.prodid IS NULL,0,1) AS isset FROM '.PRE.'_products AS a LEFT JOIN '.PRE."_products_releases AS b ON a.id=b.prodid WHERE a.active='1' AND id IN (".implode(', ', $resultIds).') GROUP BY a.id ORDER BY isset DESC, minrelease ASC '.getlimit($set['products']['searchepp']));
        } else {
            $data = $db->fetch('SELECT * FROM '.PRE."_products WHERE active='1' AND id IN (".implode(', ', $resultIds).') ORDER BY title ASC '.getlimit($set['products']['searchepp']));
        }
        $ids = get_ids($data, 'id');
...
if ('release.ASC' == $_REQUEST['sortby'] || 'release.DESC' == $_REQUEST['sortby']) {
    $data = $db->fetch('SELECT a.*,min(stamp) AS minrelease,IF(b.prodid IS NULL,0,1) AS isset FROM '.PRE.'_products AS a LEFT JOIN '.PRE."_products_releases AS b ON a.id=b.prodid WHERE a.active='1' ".$where.' GROUP BY a.id '.getorder($orderdef, 'isset DESC', 1).' '.getlimit($set['products']['epp']));
} else {
    $data = $db->fetch('SELECT * FROM '.PRE."_products WHERE active='1' ".$where.getorder($orderdef).getlimit($set['products']['epp']));
}
$ids = get_ids($data, 'id');
$types = get_ids($data, 'type');
if (count($data)) {
