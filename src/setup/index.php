...
if (!is_writeable(BASEDIR.getpath('cache'))) {
    die('cache-Ordner hat keine Schreibrechte!');
}
...
elseif (4 == $_REQUEST['step']) {
    ...
    while ($file = readdir($handle)) {
        ...
        if (!is_dir(BASEDIR.getpath('moduledir').$file)) {
            continue;
        }
        if (in_array($file, $apx->coremodules)) {
            continue;
        } //Core-Module �berspringen
        $dirs[] = $file;
...
elseif (3 == $_REQUEST['step']) {
    if ($_POST['next']) {
        if (!$_POST['username_login'] || !$_POST['username'] || !$_POST['pwd1'] || !$_POST['pwd2']) {
            message('back');
        } elseif ($_POST['pwd1'] != $_POST['pwd2']) {
            message('Passwort und Passwort-Wiederholung stimmen nicht �berein!', 'back');
        } else {
...
///////////////////////////////////////////////////////////////////////////////////////////// SCHRITT 1
    ...
    if (is_writeable(BASEDIR.getpath('uploads'))) {
        $apx->tmpl->assign('WRITEABLE_UPLOADS', 1);
    }
