...
elseif ( $_REQUEST['step']==4 ) {
	...
	while ( $file=readdir($handle) ) {
		...
		if ( in_array($file,$apx->coremodules) ) continue; //Core-Module überspringen
		$dirs[]=$file;
...
elseif ( $_REQUEST['step']==3 ) {
	if ( $_POST['next'] ) {
		...
		elseif ( $_POST['pwd1']!=$_POST['pwd2'] ) message('Passwort und Passwort-Wiederholung stimmen nicht überein!','back');
		else {
