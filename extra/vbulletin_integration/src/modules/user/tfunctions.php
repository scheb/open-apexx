...
function vbthreads_parse_ids($string) {
	...
	elseif ( strpos($string,',')===false ) return (int)$string;
...
function vbthreads_newthreads($count=5,$forumid=0,$filter=0,$template='new') {
	...
	if ( $filter ) $donot = vbthreads_open_forums(vbthreads_parse_ids($filter));
	else $donot = array();
...
function vbthreads_threads_print($from,$data,$template) {
	...
	if ( count($data) ) {
		foreach ( $data AS $res	 ) {
			...
			if ( $from=='new' ) $link=$forumurl.'showthread.php?t='.$res['threadid'];
			else $link=$forumurl.'showthread.php?goto=lastpost&t='.$res['threadid'];
